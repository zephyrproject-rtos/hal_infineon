...
* The PSOC devices user-programmable Flash consists of:
* - Application flash memory (from 2 to 8 sectors) - 128KB/256KB each.
* - EE emulation flash memory - 32KB.
*
* The XMC devices user-programmable Flash consists of:
...
*    interrupt instead of a reset.
* -# Flash write operation in PSOC devices is allowed only in one of the following CM4 states:
...
* -# For PSOC devices user must guarantee that during flash write operation no flash read
...
* sequence used.
*
* For PSOC Devices API sequence Cy_Flash_StartEraseRow() + Cy_Flash_StartProgram() there are
* four block-out regions during which Read is blocked. See <b>Figure 1</b>.
*
* For XMC devices Flash memory controller has the dual bank mode feature. When using dual bank mode,
...
*
* For XMC devices Erase ROW is not supported. User has to erase the entire sector before writing to the sector.
*
* CM7 cores in XMC devices support Data Cache. Data Cache line is 32 bytes.
...
*
* PSC devices run on single core and hence IPC is not used for erase and write operations on flash memory.
*
* <center>
...
* Write operation.
*
* In PSC devices with flash IP CY_IP_MXS40FLASHC
...
* -# For PSOC devices Flash write operation is allowed only in one of the following CM4 states:
*     -# CM4 is Active and initialized:<br>
*        call \ref Cy_SysEnableCM4 "Cy_SysEnableCM4(CY_CORTEX_M4_APPL_ADDR)".
*        <b>Note:</b> If desired user may put CM4 core in Deep Sleep any time
*        after calling Cy_SysEnableCM4().
...
*        supported.
*     .
* -# For XMC devices flash write operation is only allowed after the sector in erased.
* -# For PSOC devices use the following rules for split by sectors. (In this context, read means
...
* For more information, refer to the <b>Middleware/Infineon Em_EEPROM Middleware
...
*   <tr>
*     <td>3.100</td>
*     <td>Added support for PSOC C3 (PSC).<br>
...
*   <tr>
*     <td>3.80</td>
*     <td>Dual bank support added for PSOC devices.</td>
*     <td>Newly added APIs \ref Cy_Flashc_SetMain_Flash_Mapping , \ref Cy_Flashc_SetWork_Flash_Mapping for PSOC and XMC devices.</td>
*   </tr>
*   <tr>
*     <td>3.70</td>
...
#define CY_FLASH_ID               (CY_PDL_DRV_ID(CY_PDL_RSLT_MODULE_FLASH))                          /**< FLASH PDL ID */
...
*
* \note This function is applicable for XMC devices.
*
* \return none
*
*******************************************************************************/
...
* To avoid the Read while Write violation,
* use Cy_Flash_IsOperationComplete() to ensure flash operation is complete.
* \note This function is applicable for PSOC and PSC devices.
* \note PSC devices doesn't use IPC for any flash operations.
*
...
*
* \note In PSC devices with flash IP CY_IP_MXS40FLASHC
* When refresh is enabled, for each row an extra 16 bytes is used for storing refresh related data.
* User need to make sure that this data is not corrupted while writing and erasing the row data.
* When refresh is enabled row size has to be considered as CY_FLASH_SIZEOF_ROW + 16Bytes
*
...
* To avoid the Read while Write violation,
* use Cy_Flash_IsOperationComplete() to ensure flash operation is complete.
* \note This function is applicable for PSOC devices.
*
* \param subSectorAddr Address of the flash row number.
* The Read-while-Write violation occurs when the flash read operation is
* initiated in the same flash sector where the flash erase operation is
...
* user must clear the flash cache with the Cy_SysLib_ClearFlashCacheAndBuffer()
* function.
* \note This is added as backword compatibility function in XMC devices.
* This function will not erase the row before writing to the row. Erase ROW is not supported in XMC devices.
...
* \note This is a non blocking function and will not wait until the Write operation is complete.
*
* \note User needs to make sure that the data pointer passed to this function points to 32 byte aligned data for XMC devices.
*
* \param rowAddr Address of the flash row number.
* The Read-while-Write violation occurs when the flash read operation is
* initiated in the same flash sector where the flash write operation is
...
* the selected device is defined by the \ref CY_FLASH_SIZEOF_ROW macro. Refer to
* the device datasheet for the details.
* For XMC devices this data pointer needs to point to 32 byte aligned data.
*
* \note In PSC devices with flash IP CY_IP_MXS40FLASHC
* When refresh is enabled, for each row an extra 16 bytes is used for storing refresh related data.
* User need to make sure that this data is not corrupted while writing and erasing the row data.
...
*
* Requires a call to Cy_IPC_Sema_Init(), Cy_IPC_Pipe_Config() and
* Cy_IPC_Pipe_Init() functions before use for PSOC devices.
*
...
****************************************************************************//**
*
* \brief This function enables ECC injection and sets the flash module word address
* (not device memory address) where a parity will be injected with the parity value.
* Reports success or a reason for failure.
*
* \param address The flash module word address (FM column) where ECC parity will be injected.
*
* \param parity The parity value which will be injected.
*
* \returns none.

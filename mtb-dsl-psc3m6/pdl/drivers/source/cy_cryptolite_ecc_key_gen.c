...
/*******************************************************************************
* Function Name: Cy_Cryptolite_ECC_CheckPublicKey
****************************************************************************//*
*
...
*
* \param publicKey
* The public ECC key to validate. See \ref cy_stc_cryptolite_ecc_key.
*
* \return status code. See \ref cy_en_cryptolite_status_t.
*
*******************************************************************************/
cy_en_cryptolite_status_t Cy_Cryptolite_ECC_CheckPublicKey(CRYPTOLITE_Type *base,
        cy_stc_cryptolite_context_ecdsa_t *cfContext,
        cy_en_cryptolite_ecc_curve_id_t curveID,
        cy_stc_cryptolite_ecc_key *publicKey)
{
    cy_en_cryptolite_status_t result = CY_CRYPTOLITE_BAD_PARAMS;
    const cy_stc_cryptolite_ecc_dp_type *eccDp = NULL;
 
    /* NULL parameters checking */
    if ((base != NULL) || (cfContext != NULL) || (publicKey != NULL))
    {
        cy_stc_cryptolite_descr_t *vu_struct0 = &cfContext->vu_desptr[0];

        if( publicKey->pubkey.x == NULL || publicKey->pubkey.y == NULL )
        {
            return result;
        }
        ...
        if (eccDp != NULL)
        {
            uint8_t *lhs = cfContext->p_r;
            uint8_t *rhs = cfContext->p_s;
            uint8_t *p_u1 = cfContext->p_u1;
            uint8_t *my_P = cfContext->my_P;
            uint8_t *p_x = (uint8_t *)publicKey->pubkey.x;
            uint8_t *p_y = (uint8_t *)publicKey->pubkey.y;
            uint8_t *my_BARRETT_U = cfContext->my_BARRETT_U;
            uint32_t bytesize = VU_BITS_TO_BYTES(eccDp->size);
            uint32_t bitsize  = eccDp->size;
            cfContext->bitsize = bitsize;
            ...
            /* Check 0 < (x, y) < P */
            if (Cy_Cryptolite_Vu_test_zero(base, vu_struct0, p_x, (uint16_t)bitsize) == true ||
                Cy_Cryptolite_Vu_test_zero(base, vu_struct0, p_y, (uint16_t)bitsize) == true)
            {
                return CY_CRYPTOLITE_BAD_PARAMS;
            }
            if (Cy_Cryptolite_Vu_test_less_than(base, vu_struct0, p_x, my_P, (uint16_t)bitsize) == false)
            {
                return CY_CRYPTOLITE_BAD_PARAMS;
            }
            if (Cy_Cryptolite_Vu_test_less_than(base, vu_struct0, p_y, my_P, (uint16_t)bitsize) == false)
            {
                return CY_CRYPTOLITE_BAD_PARAMS;
            }

            /* check if x and y are on curve y^2 = x(x^2 + A) + B */
            /* compute lhs = Y^2 */
            Cy_Cryptolite_EC_SquareMod(base, cfContext, lhs, p_y, (int)bitsize); // Y^2

            /* compute rhs = x(x^2 + A) + B */
            Cy_Cryptolite_EC_SquareMod(base, cfContext, rhs, p_x, (int)bitsize); // X^2
            Cy_Cryptolite_Setnumber(p_u1, (uint8_t *)eccDp->a, bytesize); //load A
            Cy_Cryptolite_EC_AddMod(base, cfContext, rhs, rhs, p_u1); // X^2 + A
            Cy_Cryptolite_EC_MulMod(base, cfContext, rhs, rhs, p_x, (int)bitsize); // X(X^2 + A)
            Cy_Cryptolite_Setnumber(p_u1, (uint8_t *)eccDp->b, bytesize); //load B
            Cy_Cryptolite_EC_AddMod(base, cfContext, rhs, rhs, p_u1); // X(X^2 + A) + B

            /* check lhs == rhs */
            if (Cy_Cryptolite_Vu_test_equal(base, vu_struct0, lhs, rhs, (uint16_t)bitsize))
            {
                result = CY_CRYPTOLITE_SUCCESS;
            }
            else
            {
                result = CY_CRYPTOLITE_BAD_PARAMS;
            }
         }
    }

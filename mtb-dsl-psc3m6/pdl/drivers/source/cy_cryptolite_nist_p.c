                            ...
                            int size )
{
    ...
    if (Cy_Cryptolite_Vu_test_zero(base, vu_struct0, b, (uint16_t)size))
    {
                            ...
                            uint8_t * porder)
{
    ...
    /*------------------------------------------------
    * Montgomery ladder with (X,Y)-only co-Z addition (Algorithm 9)
    *------------------------------------------------
    ...
    * Doubling operation taking the affine coordinates of a point P and computing the (X,Y) only co-Z points (Q,P)
    * where Q equals [2]P. Operation start by a Jacobian doubling of P = (x1, y1, 1) which yields Q = (X2, Y2, Z) = [2]P . Afterward
    * P is updated to be co-Z to Q by P = (Z^2xP : Z^3yP : Z)
    */

    /*--------------------------------
    * IDBL: 2P and P* (algorithm 23)
      Initial Doubling Before Loop
      Let P = (x1, y1, 1), we have [2]P = (X2, Y2, Z2) where X2 = B^2-2A, Y2 = B(A - X2)-8y1^4
      and Z2 = 2y1, with A = 4x1y1^2, B = 3x1^2 + a. Note that the update of P can be deduced from
      P = (x1Z2^2: y1Z2^3: Z2) = (4x1y1^2: 8y1^4: 2y1).
    *---------------------------------
    */
    ...
    *--------------------------------
    */
    //(dc[VU_BITS_TO_BYTES(bitsize+2U)-4U] & (0x2U)) >> 1;
    carry_sizeplus1 = (uint8_t)Cy_Cryptolite_Vu_get_bit(dc, bitsize+1U);
    ...
    */
    ...
    /*--------------------------------
    co-Z doubling-addition algorithm
    ----------------------------------
    This algorithm first performs a co-Z addition (with update) between P and Q leading to a pair of co-Z points (R, P ) where R = P +Q.
    Then a conjugate co-Z addition is processed that yields the pair of co-Z points (R + P , R - P ) = (2P + Q, Q).
    Then (X, Y )-only version of the co-Z doubling-addition with update is computed.
    Input: (X1, Y1) and (X2, Y2) s.t. P = (X1 : Y1 : Z) and Q = (X2 : Y2 : Z) for some Z belong to Fq, P , Q belong to E(Fq)
    Output: (X4, Y4) and (X4', Y4' ) s.t. 2P + Q = (X4 : Y4 : Z4) and Q = (X4' : Y4' : Z4) for some Z4 belong to Fq
    1. ((X3, Y3),(X1', Y1')) = XYCZ-ADD((X1, Y1),(X2, Y2))
    2. ((X4, Y4),(X4', Y4')) = XYCZ-ADDC((X3, Y3),(X1', Y1'))
    Result : ((X4, Y4),(X4', Y4'))
    */

    Cy_Cryptolite_Vu_wait_hw(base);
    //(dc[VU_BITS_TO_BYTES(bitsize+2U)-4U] & (0x2U)) >> 1;
    carry_size = (uint8_t)Cy_Cryptolite_Vu_get_bit(dc, bitsize+1U);
    ...
    //(dc[VU_BITS_TO_BYTES(bitsize+2U)-4U] & (0x2U)) >> 1U;
    carry_sizeminus1 = (uint8_t)Cy_Cryptolite_Vu_get_bit(dc, bitsize+1U);
    (void)Cy_Cryptolite_Vu_lsl1_hw (base, vu_struct0, dc, VU_BITS_TO_WORDS(bitsize+2U), dc, VU_BITS_TO_WORDS(bitsize+2U));
    ...
    /* Compute co-z doubling-add with Update for even bits with unrolled looping
       Compute one extra for odd bit outside
    */
    odd_size = (bool)(size % 2);
    l_size = odd_size ? (size -1) : size;
    for (i = 0; i < (l_size-2); i+=2)
    {
        carry = (uint8_t)Cy_Cryptolite_Vu_get_bit(dc, bitsize+1U);//(dc[VU_BITS_TO_BYTES(bitsize+2U)-4U] & (0x2U)) >> 1;
        (void)Cy_Cryptolite_Vu_lsl1_hw (base, vu_struct1, dc, VU_BITS_TO_WORDS(bitsize+2U), dc, VU_BITS_TO_WORDS(bitsize+2U));
    ...
    P = (x, y), P' = (Xp , Yp ) s.t. (Xp/Z2, Yp/Z3) = (x, y) for some Z, and Q' = (XQ, YQ)
    Output: Q = (XQ/Z^2, YQ/Z^3)
    */

                                        ...
                                        cy_stc_cryptolite_context_sha256_t *cfContext)
{
    ...
    /* Explicitly select SHA-256 mode in the IP ver 1.2 or later.*/
    #if ((CY_IP_MXCRYPTOLITE_VERSION >=1) && (CY_IP_MXCRYPTOLITE_VERSION_MINOR >=2))
    REG_CRYPTOLITE_CTL(base) = (REG_CRYPTOLITE_CTL(base) & ~CRYPTOLITE_CTL_SHA_SEL_Msk) |
                                _VAL2FLD(CRYPTOLITE_CTL_SHA_SEL, 0x01UL);
    #endif

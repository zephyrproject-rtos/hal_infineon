...
#define CY_HPPASS_SAR_RESULT_DEINIT         (0xFFFFFFFFUL)  /* De-init value for Result registers */
#define CY_HPPASS_SAR_CHAN_DEINIT            (HPPASS_SAR_CFG_CHAN_CFG_LEFT_RIGHT_ALIGNED_Msk) /* De-init value for Chan Cfg registers */
#define CY_HPPASS_SAR_FIR_DEINIT            (HPPASS_SAR_CFG_FIR_CFG_WAIT_TAP_INIT_Msk | \
                                             HPPASS_SAR_CFG_FIR_CFG_FIR_RESULT_ALIGN_Msk) /* De-init value for FIR Cfg registers */
#else
...
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
#define CY_HPPASS_SAR_AROUTE_FW_CTRL_DEINIT (0x3FFFFFFUL)   /* De-init value for AROUTE FW Ctrl register (v3: 26 bits) */
#else
#define CY_HPPASS_SAR_AROUTE_FW_CTRL_DEINIT (0xFFFFUL)      /* De-init value for AROUTE FW Ctrl register (v1: 16 bits) */
#endif
#define CY_HPPASS_SAR_SAMPLE_TIME_DEINIT    (31UL)          /* De-init value for Sample Time register */

/* Averaged default SAR CALREFPT value, used as a fall-back when the SFLASH SAR
 * CALREFPT trim reads back as 0 on some ES100 silicon. */
#define CY_HPPASS_SAR_CALREFPT_DEFAULT      (0x148C3UL)

#define CY_HPPASS_SAR_MAX                   (4096U)             /* The ADC resolution*/
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
#define CY_HPPASS_DIFGAIN(DIF)              (1U)                /* v3: no additional gain in diff mode */
#else
...
cy_en_hppass_status_t Cy_HPPASS_SAR_Init(const cy_stc_hppass_sar_t * sarCfg)
{
...
#if defined(SFLASH_SAR_CALOFFST_0_25C)
...
#endif /* defined(SFLASH_SAR_CALOFFST_0_25C) */
    ...
    #if (CY_IP_MXS40MCPASS_VERSION >= 3u)
        HPPASS_SAR_RESULT_MASK_2(HPPASS_BASE) = 0UL;
...
void Cy_HPPASS_SAR_DeInit(void)
{
...
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
    HPPASS_SAR_RESULT_MASK_2(HPPASS_BASE) = CY_HPPASS_SAR_RESULT_DEINIT;
    HPPASS_SAR_RESULT_STATUS_2(HPPASS_BASE) = CY_HPPASS_SAR_RESULT_DEINIT;
    HPPASS_SAR_LIMIT_STATUS_2(HPPASS_BASE) = CY_HPPASS_SAR_CHAN_MSK_2;
#endif /* CY_IP_MXS40MCPASS_VERSION >= 3u */
...
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
        HPPASS_SAR_SEQ_GROUP_2(HPPASS_BASE, i) = CY_HPPASS_SAR_DEINIT;
#endif /* CY_IP_MXS40MCPASS_VERSION >= 3u */
    }
    ...
    {
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
        HPPASS_SAR_FIR_CFG(HPPASS_BASE, i) = CY_HPPASS_SAR_FIR_DEINIT;
#else
    ...
    {
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
        HPPASS_SAR_CHAN_CFG(HPPASS_BASE, i) = CY_HPPASS_SAR_CHAN_DEINIT;
#else
...
#if defined(SFLASH_SAR_CALOFFST_0_N40C)
...
#endif /* defined(SFLASH_SAR_CALOFFST_0_N40C) */
...
#if defined(SFLASH_SAR_CALOFFST_0_N40C)
    /* On some ES100 silicon the SFLASH SAR CALREFPT trim reads back as 0. Since
     * CALREFPT is a multiplicative reference point in the CALOFFST calculation, a
     * value of 0 forces every computed offset to 0 and wipes out the ADC offset
     * calibration. If the trim is missing (0), substitute the averaged default
     * CALREFPT value; otherwise use the SFLASH value as-is. */
    uint32_t calRefPt = (uint32_t)SFLASH_SAR_CALREFPT;
    if (0UL == calRefPt)
    {
        calRefPt = CY_HPPASS_SAR_CALREFPT_DEFAULT;
    }

    float refLoc = ((float)calRefPt / 65536.0f) / ((float)vRef / 1000.0f);
...
{
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
    #define MUX_SAMP_WIDTH  (6U)    /* v3: 6 virtual channels per muxed sampler */
#else
    #define MUX_SAMP_WIDTH  (4U)    /* v1: 4 virtual channels per muxed sampler */
#endif
...
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
    /* On v3, AVG_32 (5) and AVG_64 (6) hardware accumulation is capped
     * at 16 samples (same as AVG_16). Clamp the effective avg value to 4 (AVG_16)
     * so that the conversion formulas use the correct divisor. */
    if (cfg->avg > 4U)
    {
        cfg->avg = 4U;
    }
...
#endif /* CY_IP_MXS40MCPASS_VERSION >= 3u */
{
    ...
    if (CY_HPPASS_SAR_CHAN_NUM_VALID(chanIdx) && CY_HPPASS_SAR_VREF_VALID(vRef))
    {
        ...
        if (!cfg.sign && !cfg.diff)
        {
            volts = (((float32_t)(uint32_t)chanCounts + (float32_t)cfg.offset) * (float32_t)vRef * 1E-3F) /
...
int16_t Cy_HPPASS_SAR_CountsTo_mVolts(uint8_t chanIdx, uint16_t vRef, int16_t chanCounts)
{
    ...
    if (CY_HPPASS_SAR_CHAN_NUM_VALID(chanIdx) && CY_HPPASS_SAR_VREF_VALID(vRef))
    {
        ...
        if (!cfg.sign && !cfg.diff)
        {
            num = (((int32_t)(uint32_t)chanCounts + cfg.offset) * (int32_t)vRef);
        }
        else /* sign OR diff */
        {
...
#if (CY_IP_MXS40MCPASS_VERSION >= 3u)
        mVolts = (int32_t)CY_HPPASS_DIV_ROUND(num, denom);
#else
...
#endif /* CY_IP_MXS40MCPASS_VERSION >= 3u */
{
    ...
    if (CY_HPPASS_SAR_CHAN_NUM_VALID(chanIdx) && CY_HPPASS_SAR_VREF_VALID(vRef))
    {
        ...
        if (!cfg.sign && !cfg.diff)
        {
            uVolts = (int32_t)((((int64_t)(uint32_t)chanCounts + cfg.offset) * (int32_t)vRef * 1000L) /

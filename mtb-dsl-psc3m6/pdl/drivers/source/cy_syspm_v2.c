...
#define CY_SYSPM_PPCA_EPU_INDEX         (0u)
#define CY_SYSPM_PPCA_EVOUT_99          (99u)
#define CY_SYSPM_PPCA_STATUS_DISABLED   (0xFFFFFFFFUL) /**< The core is off */
#define CY_SYSPM_PPCA_STATUS_DEEPSLEEP  (0x3U)    /**< The core is in deep sleep mode */

static volatile bool irq_rdy_for_deepsleep = false;
static uint32_t cm0Status = 0UL;
static uint32_t cm1Status = 0UL;
static cy_stc_syspm_ppca_images_t ppcaImages;

static void ISR_PPCADeepSleep (void)
{
    Cy_SysPm_PPCA_ClearDeepSleepInterrupt();
    irq_rdy_for_deepsleep = true;
}

static cy_en_syspm_status_t PPCACallbackFunc(cy_stc_syspm_callback_params_t *callbackParams, cy_en_syspm_callback_mode_t mode)
{
    ...
    switch(mode)
    {
        ...
        {
            retVal = Cy_SysPm_PPCA_IsDeepSleepReady() ? CY_SYSPM_SUCCESS : CY_SYSPM_FAIL;
        }
        break;
        ...
        {
            Cy_System_PPCA_RAM_Disable();
            retVal = CY_SYSPM_SUCCESS;
        }
        break;
        case CY_SYSPM_AFTER_TRANSITION:
        {
            irq_rdy_for_deepsleep = false;
            if (cm0Status != CY_SYSPM_PPCA_STATUS_DISABLED)
            {
                PPCA_CPUSS_CNFG_MXCM330->CM33_EVENT_CTL = 0UL;
                Cy_System_Init_CPU0(ppcaImages.flash_address0, ppcaImages.image_size0);
            }
#if !defined (PPCA_CORE1_WOUNDING_ENABLED)
            if (cm1Status != CY_SYSPM_PPCA_STATUS_DISABLED)
            {
                PPCA_CPUSS_CNFG_MXCM331->CM33_EVENT_CTL = 0UL;
                Cy_System_Init_CPU1(ppcaImages.flash_address1,ppcaImages.image_size1);
            }
...
static void Configure_PPCACallbackIsr (void)
{
    static cy_stc_syspm_callback_params_t ppcaDSParams;
    static cy_stc_syspm_callback_t ppcaDSOffCallbackHandler;
    static cy_stc_syspm_callback_t ppcaDSRamCallbackHandler;
    static cy_stc_syspm_callback_t ppcaDSCallbackHandler;

    ppcaDSCallbackHandler.callback = PPCACallbackFunc;
    ppcaDSCallbackHandler.type = CY_SYSPM_DEEPSLEEP;
    ppcaDSCallbackHandler.skipMode = 0;
    ppcaDSCallbackHandler.callbackParams = &ppcaDSParams;
    ppcaDSCallbackHandler.prevItm = NULL;
    ppcaDSCallbackHandler.nextItm = NULL;
    ppcaDSCallbackHandler.order = 0;

    ppcaDSRamCallbackHandler = ppcaDSCallbackHandler;
    ppcaDSRamCallbackHandler.type = CY_SYSPM_DEEPSLEEP_RAM;
    ppcaDSOffCallbackHandler = ppcaDSCallbackHandler;
    ppcaDSOffCallbackHandler.type = CY_SYSPM_DEEPSLEEP_OFF;

    (void)Cy_SysPm_RegisterCallback(&ppcaDSCallbackHandler);
    (void)Cy_SysPm_RegisterCallback(&ppcaDSRamCallbackHandler);
    (void)Cy_SysPm_RegisterCallback(&ppcaDSOffCallbackHandler);

    cy_stc_sysint_t irq_cfg_ppca_cnfg =
    {
        .intrSrc        = ppca_cnfg_rdy_for_dslp_IRQn,
        .intrPriority   = 1UL
    };

    (void)Cy_SysInt_Init(&irq_cfg_ppca_cnfg, ISR_PPCADeepSleep);

    NVIC_DisableIRQ(irq_cfg_ppca_cnfg.intrSrc);
    irq_rdy_for_deepsleep = false;
    NVIC_ClearPendingIRQ(irq_cfg_ppca_cnfg.intrSrc);
    NVIC_EnableIRQ(irq_cfg_ppca_cnfg.intrSrc);
}

static uint32_t Check_PPCACoreStatus (uint8_t cpuNum)
{
    uint32_t cm33Status = CY_SYSPM_PPCA_STATUS_DISABLED;

    if (cpuNum == 0u)
    {
        cm33Status = PPCA_CPUSS_CNFG_MXCM330->CM33_EVENT_CTL;
        cm33Status = (cm33Status == CM33_CPU_EVENT) ?
                     Cy_SysGetCM33_0_Status() : CY_SYSPM_PPCA_STATUS_DISABLED;
    }
#if !defined (PPCA_CORE1_WOUNDING_ENABLED)
    else if (cpuNum == 1u)
    {
        cm33Status = PPCA_CPUSS_CNFG_MXCM331->CM33_EVENT_CTL;
        cm33Status = (cm33Status == CM33_CPU_EVENT) ?
                     Cy_SysGetCM33_1_Status() : CY_SYSPM_PPCA_STATUS_DISABLED;
    }
#endif /* !defined (PPCA_CORE1_WOUNDING_ENABLED) */
    else
    {
...
{
    /* 10: Clear interrupt for deep-sleep entry */
    Cy_PPCA_CNFG_ClearInterrupt(PPCA_CNFG_S2IRQ0, CY_IRQ_RDY_FOR_DEEPSLEEP);
}

bool Cy_SysPm_PPCA_IsDeepSleepReady(void)
{
    /* 8,9: PPCA CPU0/CPU1 __WFI() and ready for deep-sleep */
    uint32_t intrMasked = Cy_PPCA_CNFG_GetInterruptStatusMasked(PPCA_CNFG_S2IRQ0, CY_IRQ_RDY_FOR_DEEPSLEEP);
    irq_rdy_for_deepsleep = irq_rdy_for_deepsleep || (intrMasked != 0UL);
    Cy_SysPm_PPCA_ClearDeepSleepInterrupt();

    /* Double check core status */
    cm0Status = Check_PPCACoreStatus(0u);
    cm1Status = Check_PPCACoreStatus(1u);

    return (irq_rdy_for_deepsleep &&
            ((cm0Status == CY_SYSPM_PPCA_STATUS_DEEPSLEEP) || ((cm0Status == CY_SYSPM_PPCA_STATUS_DISABLED))) &&
            ((cm1Status == CY_SYSPM_PPCA_STATUS_DEEPSLEEP) || ((cm1Status == CY_SYSPM_PPCA_STATUS_DISABLED))));
}
...
cy_en_syspm_status_t Cy_SysPm_PPCA_RequestDeepSleep(cy_stc_syspm_ppca_images_t *coreImages)
{
    ...
    cm0Status = Check_PPCACoreStatus(0u);
    cm1Status = Check_PPCACoreStatus(1u);
    ppcaImages = *coreImages;

    /* At least one core should be available to enter deep sleep */
    if ((cm0Status == CY_SYSPM_PPCA_STATUS_DISABLED) && (cm1Status == CY_SYSPM_PPCA_STATUS_DISABLED))
    {
        retVal = CY_SYSPM_INVALID_STATE;
    }
    else if ((coreImages->flash_address0 == NULL) && (coreImages->flash_address1 == NULL))
    {
        retVal = CY_SYSPM_BAD_PARAM;
    }
    else
    {
        Configure_PPCACallbackIsr();

        /* 1: Enable clock shutdown by setting PPSS_CLK_SH_DWN_EN bit */
        Cy_PPCA_Enable(PPCA_CNFG);
        Cy_PPCA_CNFG_SetInterruptMask(PPCA_CNFG_S2IRQ0, CY_IRQ_RDY_FOR_DEEPSLEEP); /* Needed for step #9 */
        PPCA_CNFG_CTRL(PPCA_CNFG) |= PPCA_CNFG_CTRL_PPCA_CLK_SH_DWN_EN_Msk;

        cy_stc_ppca_epu_combo_config_t combo_combo_config =
        {
            .bankSel = CY_COMBO_PUT1_OUT0_PUT1_OUT1,
            .put1InEnable = CY_COMBO_ENABLE_INPUT_SIGNAL_A,
        ...
        /* 2,3,4: Configure PUT1_SEL_TEST_IN and PUT1_TEST_IN for epu_rdy_for_deepsleep signal */
        Cy_PPCA_EPU_Enable(PPCA_EPU);
        Cy_PPCA_EPU_PU_T1_Enable(PPCA_EPU_EPU, CY_SYSPM_PPCA_EPU_INDEX, CY_ENABLE_PU);
        Cy_PPCA_EPU_PU_T1_TestInConfig(PPCA_EPU_EPU, CY_SYSPM_PPCA_EPU_INDEX, true, false);

        /* 5: Configure the EPU to generate interrupt */
        Cy_PPCA_EPU_Combo_Configure( PPCA_EPU_EPU, 99, &combo_combo_config);
        Cy_PPCA_EPU_InterruptSourceSelect(PPCA_EPU_EPU_IRQ0, false, CY_SYSPM_PPCA_EVOUT_99);
        Cy_PPCA_EPU_SetInterruptMask(PPCA_EPU_EPU_IRQ0); /* 7,8: PPCA CPU0/CPU1 to do __WFI() */

        /* 6: EVOUT [99] to generate interrupt */
        Cy_PPCA_EPU_PU_T1_TestInConfig(PPCA_EPU_EPU, CY_SYSPM_PPCA_EPU_INDEX, true, true);
    }
...
void Cy_SysPm_Init(void)
{
    if(CY_SYSPM_WARM_BOOT_MODE != Cy_SysPm_GetBootMode())
    {
        ...
        (void)cy_pd_ppu_init((struct ppu_v1_reg *)CY_PPU_PPCA_SRAM0_BASE); /* Suppress a compiler warning about unused return value */
        (void)cy_pd_ppu_init((struct ppu_v1_reg *)CY_PPU_PPCA_SRAM1_BASE); /* Suppress a compiler warning about unused return value */
        (void)cy_pd_ppu_init((struct ppu_v1_reg *)CY_PPU_PPCA_SRAM2_BASE); /* Suppress a compiler warning about unused return value */
        (void)cy_pd_ppu_init((struct ppu_v1_reg *)CY_PPU_PPCA_SRAM3_BASE); /* Suppress a compiler warning about unused return value */
        (void)cy_pd_ppu_init((struct ppu_v1_reg *)CY_PPU_PPCA_SRAM4_BASE); /* Suppress a compiler warning about unused return value */
#endif
...
/* Internal helper: used only by Cy_SysPm_SystemLpActiveExit() below. Kept file
 * local and inline so the single register read is folded into the caller. */
__STATIC_INLINE bool Cy_SysPm_IsSystemLpActiveEntered(void)
{
    /* The LPACTIVE/LPSLEEP profile is reflected in hardware by the low-power
     * regulator mode bit that Cy_SysPm_SystemSetMinRegulatorCurrent() sets and
     * Cy_SysPm_SystemSetNormalRegulatorCurrent() clears. Reading it lets callers
     * determine the current profile without keeping any software state. */
    return (_FLD2BOOL(SRSS_PWR_CTL2_LINREG_LPMODE, SRSS_PWR_CTL2));
}
    ...
    else
    {
        ...
        if (retVal == CY_SYSPM_SUCCESS)
        {
            ...
            */
            ...
            for(uint32_t pllIndex = 1UL; pllIndex <= SRSS_NUM_TOTAL_PLL; pllIndex++)
            {
                if(Cy_SysClk_PllIsEnabled(pllIndex))
                {
                    (void)Cy_SysClk_PllDisable(pllIndex); /* Suppress a compiler warning about unused return value */
                }
            }
            #endif /* (defined (SRSS_NUM_TOTAL_PLL) && (SRSS_NUM_TOTAL_PLL > 0UL)) */
...
cy_en_syspm_status_t Cy_SysPm_SystemLpActiveExit(void)
{
    ...
    if (retVal == CY_SYSPM_SUCCESS)
    {
        ...
        */
        ...
        /* Sample whether the system is currently in the LPACTIVE/LPSLEEP profile
         * BEFORE Step-1 clears the low-power regulator mode bits. This replaces
         * the previous software flag with a hardware query, so Enter/Exit keep no
         * state between calls. */
        wasLpActive = Cy_SysPm_IsSystemLpActiveEntered();
        ...
        /* Step-3: High frequency clocks are restored only when the system was
         * actually in the LPACTIVE profile (that is, a matching
         * Cy_SysPm_SystemLpActiveEnter() had switched them down). Gating the
         * whole restore on wasLpActive means calling Exit without a prior Enter
         * is a clock no-op and cannot report a spurious CY_SYSPM_FAIL. */
        if (wasLpActive)
        {
            /* Re-enable the high frequency clock sources that the application
             * configured but that are currently disabled (Enter drops to the IMO
             * by disabling the FLL/PLLs). Whether a source is "configured" is
             * queried from its clock configuration registers, so no software
             * state is kept. A source that was never configured is skipped -
             * enabling it would never lock (returns CY_SYSCLK_TIMEOUT) and would
             * make this API report CY_SYSPM_FAIL. */
            if (Cy_SysClk_FllIsConfigured() && !Cy_SysClk_FllIsEnabled())
            {
            ...
            for(uint32_t pllIndex = 1UL; pllIndex <= SRSS_NUM_TOTAL_PLL; pllIndex++)
            {
                if (Cy_SysClk_PllIsConfigured(pllIndex) && !Cy_SysClk_PllIsEnabled(pllIndex))
                {
                    if (CY_SYSCLK_SUCCESS != Cy_SysClk_PllEnable(pllIndex, 10000u))
                    {
                        retVal = CY_SYSPM_FAIL;
                    }
                }
            ...
            /* Route clk_hf0 back to its configured path source (Enter forced the
             * direct IMO). If that path has no valid clock (for example a
             * pure-IMO configuration that legitimately runs on the direct IMO),
             * fall back to the direct IMO so clk_hf0 always has a source. */
            (void)Cy_SysClk_ClkHfDirectSel(0U, false);
            if (0UL == Cy_SysClk_ClkHfGetFrequency(0U))
            {
                (void)Cy_SysClk_ClkHfDirectSel(0U, true);
            }
        }
...
#if defined CY_PPU_SRAM1_BASE
            (void)cy_pd_ppu_set_power_mode((struct ppu_v1_reg *)CY_PPU_SRAM1_BASE, (uint32_t)CY_SYSTEM_SRAM_PPU_DEEPSLEEP_MODE);  /* Suppress a compiler warning about unused return value */
#endif
            retVal = CY_SYSPM_SUCCESS;
        }
        break;
...
#if defined CY_PPU_SRAM1_BASE
            (void)cy_pd_ppu_set_power_mode((struct ppu_v1_reg *)CY_PPU_SRAM1_BASE, (uint32_t)CY_SYSTEM_SRAM_PPU_DEEPSLEEP_RAM_MODE);  /* Suppress a compiler warning about unused return value */
#endif
            retVal = CY_SYSPM_SUCCESS;
        }
        break;
...
#if defined CY_PPU_SRAM1_BASE
            (void)cy_pd_ppu_set_power_mode((struct ppu_v1_reg *)CY_PPU_SRAM1_BASE, (uint32_t)CY_SYSTEM_SRAM_PPU_DEEPSLEEP_OFF_MODE);  /* Suppress a compiler warning about unused return value */
#endif
            retVal = CY_SYSPM_SUCCESS;
        }
        break;
        ...
        */
        if (retVal == CY_SYSPM_SUCCESS)
        {
            ...
            */
            ...
            #if defined (CY_IP_MXS40SSRSS)
            SRSS_PWR_CTL2 |= _VAL2FLD(SRSS_PWR_CTL2_PORBOD_LPMODE, 1u); /* DRIVERS-21917 */
            #endif /* defined (CY_IP_MXS40SSRSS) */

            if(waitFor != CY_SYSPM_WAIT_FOR_EVENT)
            {
                CY_MISRA_FP_BLOCK_START('MISRA C-2012 Rule 15.7', 1, \
                    'Workaround false positive - No non-empty terminating "else" statement')
                if (cbDeepSleepRootIdx == (uint32_t)CY_SYSPM_MODE_DEEPSLEEP_RAM)
                {
                    Cy_SysPm_StoreDSContext_Wfi();
                }
                else if (cbDeepSleepRootIdx == (uint32_t)CY_SYSPM_MODE_DEEPSLEEP_OFF)
                {
                    #if defined (CY_SYSPM_FLASHBOOT_WARMBOOT_PRESENT) && (CY_SYSPM_FLASHBOOT_WARMBOOT_PRESENT == 1UL)
                        /* Flash boot needs warmboot but the device needs to go through cold
                         * boot. On the DEEPSLEEP-OFF warmboot resume the part re-enters
                         * through the startup vector table (__Vectors). The
                         * CY_SYSPM_FLASHBOOT_WARMBOOT_PRESENT capability is derived per
                         * device in cy_device.h; parts without it neither take this warmboot
                         * entry nor provide __Vectors. */
                        Cy_Syslib_SetWarmBootEntryPoint((uint32_t*)__Vectors, true);
                        __DSB();
                    #endif
                    __WFI();
                }
                ...
                CY_MISRA_BLOCK_END('MISRA C-2012 Rule 15.7')
            }
            else
            {
                __WFE();
...
cy_en_syspm_status_t Cy_SysPm_LdoExtraRequesterConfig(cy_stc_syspm_extraReq_params_t *extraReqConfig)
{
     ...
     */
    CY_SYSPM_CORE_BUCK_OVERRIDE_ENABLE(1U);
    ...
    CY_SYSPM_CORE_BUCK_OVERRIDE_ENABLE(0U);
    CY_SYSPM_CORE_BUCK_PAUSE_ENABLE(0U);
...
{
    CY_UNUSED_PARAMETER(sramNum); /* Suppress unused-parameter on single-SRAM devices (e.g. PSC3M5) */
    ...
    */
    if (retVal == CY_SYSPM_SUCCESS)
    {
        ...
        */
         ...
         * Do not gate on Cy_SysPm_LdoGetVoltage(). It only inspects
         * PWR_TRIM_PWRSYS_CTL.ACT_REG_TRIM, while LdoSetVoltage also
         * programs PWR_TRIM_WAKE_CTL and the SRAM read-margin /
         * write-assist trims. After reset ACT_REG_TRIM may already match
         * the target while the auxiliary trims are still at reset values,
         * so the previous guard could leave the system inconsistent.
         * LdoSetVoltage is idempotent; the only cost of an unconditional
         * call is the supply-stabilisation delay.
         */
        if (Cy_SysPm_LdoIsEnabled())
        {
            /* Current active regulator is LDO */
    ...
    */
    if (retVal == CY_SYSPM_SUCCESS)
    {
        ...
        */
         ...
         * See Cy_SysPm_SystemEnterLp for rationale.
         */
        if (Cy_SysPm_LdoIsEnabled())
        {
            /* Current active regulator is LDO */
    ...
    */
    if (retVal == CY_SYSPM_SUCCESS)
    {
        ...
        */
        ...
        /* Read current active regulator and set MF voltage.
         * See Cy_SysPm_SystemEnterLp for rationale.
         */
        if (Cy_SysPm_LdoIsEnabled())
        {
    ...
    */
    if (retVal == CY_SYSPM_SUCCESS)
    {
        ...
        */
        ...
        /* Read current active regulator and set OD voltage.
         * See Cy_SysPm_SystemEnterLp for rationale. This is the path
         * that exposed the original defect: after reset ACT_REG_TRIM
         * already maps to OD, so the previous guard skipped
         * LdoSetVoltage, leaving PWR_TRIM_WAKE_CTL and the SRAM trims
         * unprogrammed.
         */
        if (Cy_SysPm_LdoIsEnabled())
        {
...
__WEAK mtb_srf_permission_s_t mtb_pdl_syspm_srf_pwrmode_permissions[] =
{
    ...
    {
        .base = (void*)CY_PPU_CPUSS_BASE,
        .sub_block = 0,
        .write_allowed = true,
    },
    {
        .base = (void*)CY_PPU_SRAM_BASE,
        .sub_block = 0,
        .write_allowed = true,
    },
};

static cy_rslt_t cy_pdl_syspm_srf_unsupported_impl_s(mtb_srf_input_ns_t* inputs_ns,
                                            ...
                                            uint8_t outputs_ptr_cnt_ns)
{
    ...
    return (cy_rslt_t)CY_SYSPM_FAIL;
}
...
mtb_srf_op_s_t _cy_pdl_syspm_srf_operations[] =
{
    ...
    {
        /* CY_PDL_SYSPM_OP_SYSCM55ENABLE - Not supported on PSC3M8 (no CM55) */
        .module_id = MTB_SRF_MODULE_PDL,
        .submodule_id = CY_PDL_SECURE_SUBMODULE_SYSPM,
        .op_id = CY_PDL_SYSPM_OP_SYSCM55ENABLE,
        .write_required = false,
        .impl = cy_pdl_syspm_srf_unsupported_impl_s,
        .input_values_len = 0UL,
        .output_values_len = 0UL,
        .input_len ={ 0UL, 0UL, 0UL },
        .needs_copy = { false, false, false },
    ...
    {
        /* CY_PDL_SYSPM_OP_SYSCM55RESET - Not supported on PSC3M8 (no CM55) */
        .module_id = MTB_SRF_MODULE_PDL,
        .submodule_id = CY_PDL_SECURE_SUBMODULE_SYSPM,
    ...
    {
        /* CY_PDL_SYSPM_OP_SYSCM55DISABLE - Not supported on PSC3M8 (no CM55) */
        .module_id = MTB_SRF_MODULE_PDL,
        .submodule_id = CY_PDL_SECURE_SUBMODULE_SYSPM,
    ...
    {
        /* CY_PDL_SYSPM_OP_SYSTRANSINITIATE - Not supported on PSC3M8 */
        .module_id = MTB_SRF_MODULE_PDL,
        .submodule_id = CY_PDL_SECURE_SUBMODULE_SYSPM,
    ...
    {
        /* CY_PDL_SYSPM_OP_SYSTRANSFINALIZE - Not supported on PSC3M8 */
        .module_id = MTB_SRF_MODULE_PDL,
        .submodule_id = CY_PDL_SECURE_SUBMODULE_SYSPM,

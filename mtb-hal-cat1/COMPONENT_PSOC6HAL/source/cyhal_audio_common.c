...
static I2S_Type *const _cyhal_audioss_base[] =
{
    ...
    I2S,
#elif (CY_IP_MXAUDIOSS_INSTANCES >= 1 && ((defined(AUDIOSS0_I2S) && AUDIOSS0_I2S) || (defined(AUDIOSS_I2S_I2S) && AUDIOSS_I2S_I2S)))
    I2S0,
#endif
...
cy_rslt_t _cyhal_audioss_init(_cyhal_audioss_t *obj, const _cyhal_audioss_pins_t* tx_pins, const _cyhal_audioss_pins_t* rx_pins, const _cyhal_audioss_config_t* config, cyhal_clock_t* clk, const _cyhal_audioss_interface_t* interface)
{
    ...
    if(CY_RSLT_SUCCESS == result && NULL != rx_pins)
    {
        ...
        else
        {
            if((obj->resource.type != CYHAL_RSC_INVALID)
                && (false == _cyhal_utils_map_resource_equal(&(obj->resource), rx_sck_map)))
            {
    ...
    {
        if(NULL == mclk_map_rx || (false == _cyhal_utils_map_resource_equal(&(obj->resource), mclk_map_rx)))
        {
            result = obj->interface->err_invalid_pin;
        }
    }
    ...
    {
        if(NULL == mclk_map_tx || (false == _cyhal_utils_map_resource_equal(&(obj->resource), mclk_map_tx)))
        {
            result = obj->interface->err_invalid_pin;
        }
    }

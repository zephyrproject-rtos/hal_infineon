...
cy_rslt_t _cyhal_comp_lp_init_hw(cyhal_comp_t *obj, const cy_stc_lpcomp_config_t* cfg)
{
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        cy_stc_sysint_t irqCfg = { _cyhal_lp_comp_irq_n[obj->resource.block_num], CYHAL_ISR_PRIORITY_DEFAULT };
        Cy_SysInt_Init(&irqCfg, _cyhal_comp_lp_irq_handler);
        NVIC_EnableIRQ(_cyhal_lp_comp_irq_n[obj->resource.block_num]);
    }
    return result;
}
...
void _cyhal_comp_lp_free(cyhal_comp_t *obj)
{
    ...
    if(CYHAL_RSC_INVALID != obj->resource.type)
    {
        ...
        {
            NVIC_DisableIRQ(_cyhal_lp_comp_irq_n[lpcomp_num]);
        }
...
void _cyhal_comp_lp_enable_event(cyhal_comp_t *obj, cyhal_comp_event_t event, uint8_t intr_priority, bool enable)
{
    ...
    IRQn_Type irqn = _cyhal_lp_comp_irq_n[obj->resource.block_num];

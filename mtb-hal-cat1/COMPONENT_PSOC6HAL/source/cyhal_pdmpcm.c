...
#if (defined(AUDIOSS_PDM) && AUDIOSS_PDM) || (defined(AUDIOSS0_PDM) && AUDIOSS0_PDM)
...
#if (defined(AUDIOSS1_PDM) && AUDIOSS1_PDM)
...
#if (_CYHAL_PDM_INST0)
static IRQn_Type _cyhal_pdm_pcm_irq_n0[] =
{
#if (CY_IP_MXAUDIOSS_INSTANCES == 1)
...
#if (_CYHAL_PDM_INST1)
static IRQn_Type _cyhal_pdm_pcm_irq_n1[] =
{
#if defined(CY_IP_MXAUDIOSS)
...
static void _cyhal_pdm_pcm_get_block_from_irqn(IRQn_Type irqn, uint8_t* block, uint8_t* channel)
{
    switch (irqn)
    {
...
#endif
{
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        IRQn_Type irqn = _cyhal_pdm_pcm_irq_n[obj->resource.block_num][obj->resource.channel_num];
...
void cyhal_pdm_pcm_free(cyhal_pdm_pcm_t *obj)
{
    ...
    if (CYHAL_RSC_INVALID != obj->resource.type)
    {
        ...
        if(NULL != obj->base)
        {
            ...
            NVIC_DisableIRQ(_cyhal_pdm_pcm_irq_n[obj->resource.block_num][obj->resource.channel_num]);
        }
...
cy_rslt_t cyhal_pdm_pcm_read_async(cyhal_pdm_pcm_t *obj, void *data, size_t length)
{
    ...
    NVIC_EnableIRQ(_cyhal_pdm_pcm_irq_n[obj->resource.block_num][obj->resource.channel_num]);
    return CY_RSLT_SUCCESS;
}
...
void cyhal_pdm_pcm_enable_event(cyhal_pdm_pcm_t *obj, cyhal_pdm_pcm_event_t event, uint8_t intr_priority, bool enable)
{
    ...
    NVIC_SetPriority(_cyhal_pdm_pcm_irq_n[obj->resource.block_num][obj->resource.channel_num], intr_priority);
}

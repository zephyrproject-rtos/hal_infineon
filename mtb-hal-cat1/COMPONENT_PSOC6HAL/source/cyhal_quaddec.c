...
static cy_rslt_t _cyhal_quadec_pin_init(cyhal_quaddec_t *obj, cyhal_gpio_t pin, cyhal_gpio_t *obj_pin, cyhal_signal_type_t signal_type, cyhal_quaddec_input_t input)
{
    if (NC == pin)
    {
                                 ...
                                 uint32_t frequency /* Only required if clk == NULL */)
{
    ...
    if (CY_RSLT_SUCCESS == rslt)
    {
        en_clk_dst_t pclk =(en_clk_dst_t)(_CYHAL_TCPWM_DATA[
            obj->tcpwm.resource.block_num].clock_dst + obj->tcpwm.resource.channel_num);
                             ...
                             const cyhal_clock_t *clk, uint32_t frequency)
{
    ...
    if (rslt == CY_RSLT_SUCCESS)
    {
        ...
        if (rslt == CY_RSLT_SUCCESS)
        {
            ...
            do
            {
                ...
                _cyhal_quaddec_get_phy_a_input_dest_trig_idx < _CYHAL_TCPWM_TRIGGER_INPUTS_PER_BLOCK);
        }
    }
    #else
    ...
    if (rslt == CY_RSLT_SUCCESS)
    {
        #if defined(COMPONENT_CAT1A) || defined(COMPONENT_CAT1B) // already initialized above
        ...
        if (rslt == CY_RSLT_SUCCESS)
        {
            rslt = _cyhal_quadec_pin_init(obj, phi_b, &(obj->phi_b), CYHAL_SIGNAL_TYPE_LEVEL, CYHAL_QUADDEC_INPUT_PHI_B);
        }

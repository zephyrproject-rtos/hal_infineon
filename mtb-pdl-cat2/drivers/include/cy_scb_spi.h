...
*
* \snippet scb_spi_snippet.c SPI_CFG
*
********************************************************************************
...
*
* \snippet scb_spi_snippet.c SPI_CFG_PINS_DEFINES
* \snippet scb_spi_snippet.c SPI_CFG_PINS
*
********************************************************************************
...
*
* \snippet scb_spi_snippet.c SPI_CFG_ASSIGN_CLOCK
*
********************************************************************************
...
* API to do that.
*
* \snippet scb_spi_snippet.c SPI_CFG_DATA_RATE_SLAVE
*
...
*
* \snippet scb_spi_snippet.c SPI_CFG_DATA_RATE_MASTER
*
...
*
* \snippet scb_spi_snippet.c SPI_INTR_A
* \snippet scb_spi_snippet.c SPI_INTR_B
*
********************************************************************************
...
*
* \snippet scb_spi_snippet.c SPI_ENABLE
*
********************************************************************************
...
*
* \snippet scb_spi_snippet.c SPI_TRANFER_DATA_LL
*
********************************************************************************
...
*
* \snippet scb_spi_snippet.c SPI_TRANFER_DATA
*
********************************************************************************
...
*
* The SPI master is disabled during Deep Sleep and stops driving
...
* Sleep mode. To do that, write the GPIO data register of each pin
...
* SPI master control of the pins . Copy \ref Cy_SCB_SPI_DeepSleepCallback and
* as appropriate, and make the changes described above inside the function.
* Alternately, external pull-up or pull-down resistors can be connected to
* the appropriate SPI lines to keep them inactive during Deep-Sleep.
*
********************************************************************************
* \subsection group_scb_spi_DsClockConfig_callback Deep Sleep Clock Config Callback
********************************************************************************
...
* \ref Cy_SCB_SPI_RegisterDSClockConfig.
*
* The callback function can be generated by using the Device Configurator.
*
* \snippet scb_spi_snippet.c SPI_DS_REGISTER_CLOCK_CONFIG_CALLBACK
...
typedef enum
{
    CY_SCB_SPI_SS_SETUP_DELAY_0_75_CYCLES = 0U,     /**< 0.75 SPI clock cycles */
    CY_SCB_SPI_SS_SETUP_DELAY_1_75_CYCLES = 1U,     /**< 1.75 SPI clock cycles */
...
typedef enum
{
    CY_SCB_SPI_SS_HOLD_DELAY_0_75_CYCLES = 0U,      /**< 0.75 SPI clock cycles */
    CY_SCB_SPI_SS_HOLD_DELAY_1_75_CYCLES = 1U,      /**< 1.75 SPI clock cycles */
...
typedef enum
{
    CY_SCB_SPI_SS_INTERFRAME_DELAY_1_5_CYCLES = 0U,     /**< 1.5 SPI clock cycles */
    CY_SCB_SPI_SS_INTERFRAME_DELAY_2_5_CYCLES = 1U,     /**< 2.5 SPI clock cycles */
...
/**
* Provides the typedef for the callback function called in the
* \ref Cy_SCB_SPI_DeepSleepCallback to notify the user when going into
* and out of deepsleep \ref group_scb_spi_macros_deep_sleep_callback_events.
*/
typedef void (* cy_cb_scb_spi_handle_deep_sleep_mode_t)(uint32_t event);
...
__STATIC_INLINE uint32_t Cy_SCB_SPI_WriteArray(CySCB_Type *base, void const *buffer, uint32_t size);
...
* in \ref CY_SCB_SPI_CPHA0_CPOL0 and \ref CY_SCB_SPI_CPHA1_CPOL0 SCLK modes.
*/
...
#define CY_SCB_SPI_RX_ERR_PARITY    (SCB_INTR_RX_PARITY_ERROR_Msk)
...
* (the TX FIFO buffer is empty and no additional data is requested by the Master)
* in \ref CY_SCB_SPI_CPHA0_CPOL0 and \ref CY_SCB_SPI_CPHA1_CPOL0 SCLK modes.
* On the Slave side, the application can ignore the first interrupt. On the
* Master side, the packet checksum can be used on the application level to verify
* if the underflow interrupt is valid.
*/
...
* transfer error and parity error. Check \ref Cy_SCB_SPI_GetTransferStatus.
*/
...
*/
#define CY_SCB_SPI_SLAVE_DEEP_SLEEP_ENTER_EVENT (0x01UL)

/**
* Indicates on exit from Deep Sleep mode.
*/
#define CY_SCB_SPI_SLAVE_DEEP_SLEEP_EXIT_EVENT  (0x02UL)
...
#else /* CY_IP_MXSCB */
#define CY_SCB_SPI_RX_INTR_MASK  (CY_SCB_SPI_RX_TRIGGER  | CY_SCB_SPI_RX_NOT_EMPTY | CY_SCB_SPI_RX_FULL | \
                                  CY_SCB_SPI_RX_OVERFLOW | CY_SCB_SPI_RX_UNDERFLOW | CY_SCB_SPI_RX_ERR_PARITY)

#define CY_SCB_SPI_TRANSFER_ERR    (CY_SCB_SPI_SLAVE_TRANSFER_BUS_ERR | CY_SCB_SPI_TRANSFER_OVERFLOW | \
                                    CY_SCB_SPI_TRANSFER_UNDERFLOW | CY_SCB_SPI_TRANSFER_PARITY_ERR)
#endif /* CY_IP_M0S8SCB */
...
#ifdef CY_IP_MXSCB
#define CY_SCB_SPI_IS_PARITY_VALID(parity)      ( (CY_SCB_SPI_PARITY_NONE == (parity)) || \
                                                  (CY_SCB_SPI_PARITY_EVEN == (parity)) || \
                                                  (CY_SCB_SPI_PARITY_ODD  == (parity)) )
#endif /* CY_IP_MXSCB */
...
#ifdef CY_IP_M0S8SCB
    CY_REG32_CLR_SET(SCB_SPI_CTRL(base), SCB_SPI_CTRL_SLAVE_SELECT, (uint32_t) slaveSelect);
#else /* CY_IP_MXSCB */
...
#if defined (CY_IP_M0S8SCB) || defined (CY_DOXYGEN)
/*******************************************************************************
* Function Name: Cy_SCB_SPI_RegisterDSClockConfig
****************************************************************************//**
*
* Registers a callback function that will occur in \ref
* Cy_SCB_SPI_DeepSleepCallback when the SPI goes into deepsleep. In the
...
*
* \param callback
* The pointer to a callback function that will enable and disable the SPI clock
* divider when going into and out of deepsleep.
* See \ref cy_cb_scb_spi_handle_events_t for the function prototype.
*
* \param context
* The pointer to the context structure \ref cy_stc_scb_spi_context_t allocated
...
__STATIC_INLINE void Cy_SCB_SPI_RegisterDSClockConfig(CySCB_Type const *base,
            cy_cb_scb_spi_handle_deep_sleep_mode_t callback, cy_stc_scb_spi_context_t *context)
{
    /* Suppress a compiler warning about unused variables */
    CY_UNUSED_PARAMETER(base);

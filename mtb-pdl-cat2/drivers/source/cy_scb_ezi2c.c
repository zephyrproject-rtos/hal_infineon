...
* * <b>enable-wakeup</b>: on the incoming EZI2C slave address, the slave
...
* * <b>Not enable-wakeup</b>: the EZI2C is disabled. It is enabled
...
cy_en_syspm_status_t Cy_SCB_EZI2C_DeepSleepCallback(cy_stc_syspm_callback_params_t *callbackParams, cy_en_syspm_callback_mode_t mode)
{
    ...
    switch (mode)
    {
        case CY_SYSPM_CHECK_READY:
        {
            ...
            /* If the EZI2C is not busy and if the EZI2C is in the IDLE state,
            * it is ready for Deep Sleep mode. Otherwise, it returns fail and
            * restores the slave interrupt sources.
            */
#ifdef CY_IP_M0S8SCB
            if (CY_SCB_EZI2C_STATE_IDLE == locContext->state)
#else /* CY_IP_MXSCB */
            if ((CY_SCB_EZI2C_STATE_IDLE == locContext->state) && (!_FLD2BOOL(SCB_I2C_STATUS_I2CS_IC_BUSY, SCB_I2C_STATUS(locBase))))
#endif /* CY_IP_M0S8SCB */
            {
                ...
                {
                    /* The SCB is NOT configured as enable-wakeup: disable the EZI2C.
        ...
        case CY_SYSPM_CHECK_FAIL:
        {
            ...
            */
            ...
            if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
            {
                /* The SCB is configured as enable-wakeup: restore the slave interrupt
                * sources.
                */
                Cy_SCB_SetSlaveInterruptMask(locBase, CY_SCB_EZI2C_SLAVE_INTR);
            }
            ...
            {
                /* The SCB is NOT configured as enable-wakeup: enable the slave to operate. */
            ...
            */
            ...
            if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
            {
                ...
                */
                ...
                {
                    locContext->cbDeepSleep(CY_SCB_EZI2C_SLAVE_DEEP_SLEEP_ENTER_EVENT);
                }
#else /* CY_IP_MXSCB */
                /* Disable SCB clock */
        ...
        case CY_SYSPM_AFTER_TRANSITION:
        {
            if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
            {
                ...
                if (NULL != locContext->cbDeepSleep)
                {
                    locContext->cbDeepSleep(CY_SCB_EZI2C_SLAVE_DEEP_SLEEP_EXIT_EVENT);
                }
#else /* CY_IP_MXSCB */
                /* Enable SCB clock */
                SCB_CTRL(locBase) &= (uint32_t) ~SCB_CTRL_EC_ACCESS_Msk;
...
/*******************************************************************************
* Function Name: UpdateRxFifoLevel
****************************************************************************//**
*
...
static void HandleDataReceive(CySCB_Type *base, cy_stc_scb_ezi2c_context_t *context)
{
    switch(context->state)
    {
        ...
        default:
            /* State is unknown, possible context corruption */
        break;
    }
}

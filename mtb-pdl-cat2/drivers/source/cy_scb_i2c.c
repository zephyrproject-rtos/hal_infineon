...
cy_en_scb_i2c_status_t Cy_SCB_I2C_Init(CySCB_Type *base, cy_stc_scb_i2c_config_t const *config, cy_stc_scb_i2c_context_t *context)
{
    ...
    else
    {
...
#ifdef CY_IP_MXSCB
        bool enableSStretchHs = (notMasterMode) && (config->hsEnable);

        /* Configure the HS mode */
        SCB_I2C_CTRL_HS(base) = _BOOL2FLD(SCB_I2C_CTRL_HS_HS_ENABLED, config->hsEnable);
#endif /* CY_IP_MXSCB */

        {
            ...
            SCB_RX_CTRL(base)      = _BOOL2FLD(SCB_RX_CTRL_MEDIAN, enableDigFilter) | CY_SCB_I2C_RX_CTRL;
...
#ifdef CY_IP_MXSCB
        /* Configure HS interrupt sources*/
        SCB_INTR_S_MASK(base) |= ((config->hsEnable) && (notMasterMode)) ? CY_SCB_I2C_SLAVE_INTR_HS : 0UL;
#endif /* CY_IP_MXSCB */
...
* * <b>Enable-wakeup</b>: during Deep Sleep mode on incoming I2C slave address
...
*   mode.
* * <b>Not enable-wakeup</b>: the SCB is disabled in Deep Sleep mode.
    ...
    switch(mode)
    {
...
#else /* CY_IP_MXSCB */
            /* If the I2C is in the IDLE state and external master hasn't access
            * to the internally clocked slave logic Deep Sleep mode (either the
            * master or the slave is not busy), otherwise return fail and restore
            * the slave interrupt sources.
            */
            if ((CY_SCB_I2C_IDLE == locContext->state) && (!_FLD2BOOL(SCB_I2C_STATUS_I2CS_IC_BUSY, SCB_I2C_STATUS(locBase))))
#endif /* CY_IP_M0S8SCB */
            {
                if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
                {
                    /* The SCB is configured as enable-wakeup: do not restore the address
                ...
                {
                    /* The SCB is NOT configured as enable-wakeup: disable the I2C. The slave
...
#else /* CY_IP_MXSCB */
                    Cy_SCB_SetSlaveInterruptMask(locBase, locContext->hsEnable ? (CY_SCB_I2C_SLAVE_INTR | CY_SCB_I2C_SLAVE_INTR_HS) : CY_SCB_I2C_SLAVE_INTR);
#endif /* CY_IP_M0S8SCB */
                }
        ...
        case CY_SYSPM_CHECK_FAIL:
        {
            ...
            */
            ...
            if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
            {
                /* The SCB is configured as enable-wakeup: restore the slave interrupt sources */
#ifdef CY_IP_M0S8SCB
                    Cy_SCB_SetSlaveInterruptMask(locBase, CY_SCB_I2C_SLAVE_INTR);
#else /* CY_IP_MXSCB */
                    Cy_SCB_SetSlaveInterruptMask(locBase, locContext->hsEnable ? (CY_SCB_I2C_SLAVE_INTR | CY_SCB_I2C_SLAVE_INTR_HS) : CY_SCB_I2C_SLAVE_INTR);
            ...
            {
                /* The SCB is NOT configured as enable-wakeup: enable the I2C to operate */
                Cy_SCB_I2C_Enable(locBase, locContext);
            }
        ...
        case CY_SYSPM_BEFORE_TRANSITION:
        {
            ...
            */
            ...
            if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
            {
                /* The SCB is configured as enable-wakeup: enable the I2C wakeup interrupt
                ...
                */
                ...
                {
                    locContext->cbDeepSleep(CY_SCB_I2C_SLAVE_DEEP_SLEEP_ENTER_EVENT);
                }
#else /* CY_IP_MXSCB */
                /* Disable SCB clock */
        ...
        case CY_SYSPM_AFTER_TRANSITION:
        {
            if (_FLD2BOOL(SCB_CTRL_EC_AM_MODE, SCB_CTRL(locBase)))
            {
                ...
                if (NULL != locContext->cbDeepSleep)
                {
                    locContext->cbDeepSleep(CY_SCB_I2C_SLAVE_DEEP_SLEEP_EXIT_EVENT);
                }
#else /* CY_IP_MXSCB */
                /* Enable SCB clock */
                SCB_CTRL(locBase) &= (uint32_t) ~SCB_CTRL_EC_ACCESS_Msk;
                ...
                /* The SCB is configured as enable-wakeup: disable the I2C wakeup interrupt
...
/*******************************************************************************
* Function Name: Cy_SCB_I2C_GetDataRateHS
****************************************************************************//**
*
* Returns the High Speed data rate for the selected SCB block
*
* \param base
* The pointer to the I2C SCB instance.
*
...
uint32_t Cy_SCB_I2C_GetDataRateHS(CySCB_Type const *base, uint32_t scbClockHz)
{
    ...
    {
        /* Check the clock frequency range to get maximum supported data rate */
        if ((scbClockHz >= CY_SCB_I2C_SLAVE_HS_CLK_MIN_3400) && (scbClockHz <= CY_SCB_I2C_SLAVE_HS_CLK_MAX_3400))
        {
            actualDataRate = CY_SCB_I2C_HS_DATA_RATE_3400;
        }
        else if ((scbClockHz >= CY_SCB_I2C_SLAVE_HS_CLK_MIN_1700) && (scbClockHz <= CY_SCB_I2C_SLAVE_HS_CLK_MAX_1700))
        {
            actualDataRate = CY_SCB_I2C_HS_DATA_RATE_1700;
        }
        else
        {
    ...
    {
        /* Master mode is not supports High-speed mode */
    }
...
cy_en_scb_i2c_status_t Cy_SCB_I2C_MasterRead(CySCB_Type *base,
                            cy_stc_scb_i2c_master_xfer_config_t const *xferConfig,
                            cy_stc_scb_i2c_context_t *context)
{
    ...
    if (0UL != (CY_SCB_I2C_IDLE_MASK & context->state))
    {
        ...
        if ((context->useRxFifo) && (!context->masterPause) && (context->masterBufferSize >= 2UL))
        {
            ...
            Cy_SCB_SetRxFifoLevel(base, (context->masterBufferSize <= CY_SCB_I2C_FIFO_SIZE) ?
                                        (context->masterBufferSize - 2UL) : ((CY_SCB_I2C_FIFO_SIZE / 2UL) - 1UL));
    ...
    Cy_SCB_SetSlaveInterruptMask(base, CY_SCB_I2C_SLAVE_INTR);
#else /* CY_IP_MXSCB */
    Cy_SCB_SetSlaveInterruptMask(base, context->hsEnable ? (CY_SCB_I2C_SLAVE_INTR | CY_SCB_I2C_SLAVE_INTR_HS) : CY_SCB_I2C_SLAVE_INTR);
#endif /* CY_IP_M0S8SCB */
...
#ifdef CY_IP_MXSCB
    /* Handle the Hs event*/
    if (0UL != (CY_SCB_I2C_SLAVE_INTR_HS & slaveIntrStatus))
    {
        SlaveHandleHsMode(base, context);
    }
#endif /* CY_IP_MXSCB */
...
#ifdef CY_IP_MXSCB
/*******************************************************************************
* Function Name: SlaveHandleHsMode
****************************************************************************//**
*
* Change slave mode from F/S to High-speed mode and vice versa
*
* \param base
* The pointer to the I2C SCB instance.
*
...
static void SlaveHandleHsMode(CySCB_Type *base, cy_stc_scb_i2c_context_t *context)
{
    if (NULL != context->cbHsMode)
    {
        if (_FLD2BOOL(SCB_I2C_STATUS_HS_MODE, SCB_I2C_STATUS(base)))
        {
            /* Call Hs callback */
            context->cbHsMode(CY_SCB_I2C_SLAVE_HS_ENTER_EVENT);

            /* Set automatically ACK/NACK on slave address and received data */
            SCB_I2C_CTRL(base) |= SCB_I2C_CTRL_S_READY_ADDR_ACK_Msk      |
                                  SCB_I2C_CTRL_S_NOT_READY_DATA_NACK_Msk |
                                  SCB_I2C_CTRL_S_READY_DATA_ACK_Msk      |
                                  SCB_I2C_CTRL_S_NOT_READY_DATA_NACK_Msk;

            /* Set RX fifo level half fifo size */
            Cy_SCB_SetRxFifoLevel(base, CY_SCB_I2C_HALF_FIFO_SIZE);

            /* Update Slave status */
            context->slaveStatus |= CY_SCB_I2C_SLAVE_HS_MODE;

            /* Disable stretching SCL line after Start, Master code, NACK */
            SCB_I2C_S_CMD(base) &= ~SCB_I2C_S_CMD_S_STRETCH_HS_Msk;

            Cy_SCB_ClearSlaveInterrupt(base, CY_SCB_SLAVE_INTR_I2C_HS_ENTER);
        }
        else
        {
            /* Call Hs callback */
            context->cbHsMode(CY_SCB_I2C_SLAVE_HS_EXIT_EVENT);
    ...
    Cy_SCB_ClearSlaveInterrupt(base, CY_SCB_I2C_SLAVE_INTR_HS);
...
{
#ifdef CY_IP_MXSCB
    bool hsMode = CY_SCB_I2C_SLAVE_HS_MODE == (context->slaveStatus & CY_SCB_I2C_SLAVE_HS_MODE);
        ...
        else
        {
            ...
            if (context->slaveRxBufferSize > 0UL)
            {
                ...
                if (context->slaveRxBufferSize > CY_SCB_I2C_FIFO_SIZE)
                {
                    ...
                    * Level is set so that reduce the amount of data lost */

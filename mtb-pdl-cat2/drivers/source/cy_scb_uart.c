...
/* Static functions */
static void UartHandleDataReceive  (CySCB_Type *base, cy_stc_scb_uart_context_t *context);
    ...
    else
    {
...
#ifdef CY_IP_M0S8SCB
        SCB_CTRL(base) |= _BOOL2FLD(SCB_CTRL_BYTE_MODE, (config->dataWidth <= CY_SCB_BYTE_WIDTH));
#else /* CY_IP_MXSCB */
        ...
        */
        ...
        SCB_UART_RX_CTRL(base) = _BOOL2FLD(SCB_UART_RX_CTRL_POLARITY, config->irdaInvertRx)                  |
                                 ...
                                 _BOOL2FLD(SCB_UART_RX_CTRL_DROP_ON_FRAME_ERROR, config->dropOnFrameError)   |
                                 _VAL2FLD(SCB_UART_RX_CTRL_BREAK_WIDTH, (config->breakWidth - 1UL))          |
                                 _BOOL2FLD(SCB_UART_RX_CTRL_BREAK_LEVEL, config->breakLevel)                 |
                                 _VAL2FLD(SCB_UART_RX_CTRL_STOP_BITS,   ((uint32_t) config->stopBits) - 1UL) |
                                 _VAL2FLD(CY_SCB_UART_RX_CTRL_SET_PARITY, (uint32_t) config->parity);
#endif /* CY_IP_M0S8SCB */
...
cy_en_scb_uart_status_t Cy_SCB_UART_Receive(CySCB_Type *base, void *buffer, uint32_t size, cy_stc_scb_uart_context_t *context)
{
    ...
    if (0UL == (context->rxStatus & CY_SCB_UART_RECEIVE_ACTIVE))
    {
        ...
        if (locSize > 0UL)
        {
            ...
            context->rxBufSize = locSize;
            context->rxBufIdx =  numToCopy;

            /* Set the RX FIFO level to the trigger interrupt */
            Cy_SCB_SetRxFifoLevel(base, (locSize > irqRxLevel) ? (irqRxLevel - 1UL) : (locSize - 1UL));
...
* Blocks until break is completed. Only call this function if previous transfer
* is complete: UART TX FIFO and shifter are empty and CY_SCB_TX_INTR_UART_DONE
* interrupt was trigger.
*
* \param base
* The pointer to the UART SCB instance.
*
...
void Cy_SCB_UART_SendBreakBlocking(CySCB_Type *base, uint32_t breakWidth)
{
    ...
    CY_REG32_CLR_SET(SCB_TX_CTRL(base), SCB_TX_CTRL_DATA_WIDTH, (breakWidth - 2UL));
...
void Cy_SCB_UART_Interrupt(CySCB_Type *base, cy_stc_scb_uart_context_t *context)
{
    if (0UL != (CY_SCB_RX_INTR & Cy_SCB_GetInterruptCause(base)))
    {
        ...
        if (0UL != (CY_SCB_RX_INTR_LEVEL & Cy_SCB_GetRxInterruptStatusMasked(base)))
        {
            ...
            {
                UartHandleDataReceive(base, context);
            }
            else
            {
                if (NULL != context->rxRingBuf)
                {
                    UartHandleRingBuffer(base, context);
                }
            }
    ...
    if (0UL != (CY_SCB_TX_INTR & Cy_SCB_GetInterruptCause(base)))
    {
        ...
        {
            UartHandleDataTransmit(base, context);
...
/*******************************************************************************
* Function Name: UartHandleDataReceive
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: UartHandleRingBuffer
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: UartHandleDataTransmit
****************************************************************************//**
*
...
static void UartHandleDataTransmit(CySCB_Type *base, cy_stc_scb_uart_context_t *context)
{
    ...
    if ((fifoSize != Cy_SCB_GetNumInTxFifo(base)) && (1UL == context->txBufSize))
    {
        ...
        txData = (uint32_t) ((byteMode) ? ((uint8_t *)  context->txBuf)[0UL] : ((uint16_t *) context->txBuf)[0UL]);

...
#define CY_SEGLCD_IS_ENABLED(base)          (0UL != (LCD_CONTROL(base) & LCD_CONTROL_HS_EN_Msk))
...
cy_en_seglcd_status_t Cy_SegLCD_Init(LCD_Type * base, cy_stc_seglcd_config_t const * config)
{
    ...
    if (NULL != base)
    {
        ...
        if ((locSubfr <= CY_SEGLCD_DIV_MAX) && (locDead <= CY_SEGLCD_DIV_MAX))
        {
            ...
            LCD_CONTROL(base) = _VAL2FLD(LCD_CONTROL_LCD_MODE, CY_SEGLCD_SPEED_MODE) |

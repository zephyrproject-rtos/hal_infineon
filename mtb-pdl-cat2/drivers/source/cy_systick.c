...
void Cy_SysTick_Init(cy_en_systick_clock_source_t clockSource, uint32_t interval)
{
    ...
    __RAM_VECTOR_TABLE[CY_SYSTICK_IRQ_NUM] = &Cy_SysTick_ServiceCallbacks;
...
{
    CY_REG32_CLR_SET(SYSTICK_CTRL, SysTick_CTRL_CLKSOURCE, clockSource);
}
...
{
    cy_en_systick_clock_source_t result;

    if (_FLD2VAL(SysTick_CTRL_CLKSOURCE, SYSTICK_CTRL) == 0U)
    {
        result = CY_SYSTICK_CLOCK_SOURCE_CLK_LF;
    }
    else
    {
        result = CY_SYSTICK_CLOCK_SOURCE_CLK_CPU;
    }

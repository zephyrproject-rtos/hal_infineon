...
/* The VDDD threshold value that decides whether Regulator shall be
 * turned on for the USB Device block.
 */
#define USBDSS_VDDD_THRESHOLD   (3700U)
/*******************************************************************************
                                                   ...
                                                   cy_stc_usbfs_dev_drv_context_t      *context)
{
    ...
    /* Enables clock */
    USBFS_DEV_USB_CLK_EN(base) = USBFS_USB_CLK_EN_CSR_CLK_EN_Msk;
    ...
    USBFS_DEV_CR1(base) |= (USBDEV_CR1_ENABLE_LOCK);

    /* Clears register (except reserved): the IOMODE = 0 means usb IP controls the usb pins */
    USBFS_DEV_USBIO_CR1(base) = (USBFS_DEV_USBIO_CR1(base) & USBFS_USBIO_CR1_USBPUEN_Msk);
    ...
    USBFS_DEV_POWER_CTL(base) = (USBFS_BCD_POWER_CTRL_SUSPEND_Msk        |
                                    USBFS_BCD_POWER_CTRL_SUSPEND_DEL_Msk     |
                                    USBFS_BCD_POWER_CTRL_ENABLE_RCVR_Msk     |
                                    USBFS_BCD_POWER_CTRL_VBUS_VALID_OVR_Msk  |
                                    USBFS_BCD_POWER_CTRL_ENABLE_Msk          |
                                    USBFS_BCD_POWER_CTRL_ENABLE_DPO_Msk      |
                                    USBFS_BCD_POWER_CTRL_ENABLE_DMO_Msk
                                    );
    ...
    /* When taking PHY out of suspend mode, first clear SUSPEND, then clear SUSPEND_DEL at least 2us later */
    USBFS_DEV_POWER_CTL(base) &= ~USBFS_BCD_POWER_CTRL_SUSPEND_Msk;

    Cy_SysLib_DelayUs(WAIT_SUSPEND_DISABLE);

    USBFS_DEV_POWER_CTL(base) &= ~USBFS_BCD_POWER_CTRL_SUSPEND_DEL_Msk;

    /* Configures the level selection (HI, MED, LO) for each interrupt source */
    USBFS_DEV_INTR_LVL_SEL(base) = config->intrLevelSel;

    /* Enables the interrupt sources: Bus Reset and EP0 */
    USBFS_DEV_INTR_SIE_MASK(base) = (USBFS_BCD_INTR_SIE_BUS_RESET_INTR_Msk |
                                         USBFS_BCD_INTR_SIE_EP0_INTR_Msk);

    /* Clears the LPM register (disable LPM response) */
    USBFS_DEV_LPM_CTL(base) = 0UL;
    ...
    switch(config->mode)
    {
        case CY_USBFS_DEV_DRV_EP_MANAGEMENT_CPU:
        {
            ...
            USBFS_DEV_ARB_CFG(base) = _VAL2FLD(USBFS_ARB_CFG_DMA_CFG, CY_USBFS_DEV_DRV_EP_MANAGEMENT_CPU);
...
void Cy_USBFS_Dev_Drv_DeInit(USBFS_Type *base, cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    USBFS_DEV_POWER_CTL(base)     = 0UL;
    USBFS_DEV_USBIO_CTL(base)     = 0UL;
    USBFS_DEV_FLOW_CTL(base)      = 0UL;
    USBFS_DEV_LPM_CTL(base)       = 0UL;
    USBFS_DEV_INTR_SIE(base)      = 0UL;
    USBFS_DEV_INTR_SIE_SET(base)  = 0UL;
    USBFS_DEV_INTR_SIE_MASK(base) = 0UL;
    USBFS_DEV_INTR_LVL_SEL(base)  = 0UL;
...
/*******************************************************************************
* Function Name: Cy_USBFS_Dev_RegEnable
****************************************************************************//**
*
* Enables the internal regulator of the USBFS Block. The Regulator needs to
* be enabled if the VDDD is higher than 3.7V.
*
* \param base
* The pointer to the USBFS instance.
*
...
void Cy_USBFS_Dev_Drv_RegEnable(USBFS_Type *base,cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    USBFS_DEV_CR1(base) = (USBDEV_CR1_REG_ENABLE);
    USBFS_DEV_USBIO_TRIM(base) = USBIO_TRIM_REG_EN_VALUE;
}

/*******************************************************************************
* Function Name: Cy_USBFS_Dev_RegDisable
****************************************************************************//**
*
* Disables the internal regulator of the USBFS Block. The Regulator needs to
* be disabled if the VDDD is lesser than 3.7V.
*
* \param base
* The pointer to the USBFS instance.
*
...
{
    CY_UNUSED_PARAMETER(context);
    USBFS_DEV_USBIO_TRIM(base) = USBIO_TRIM_REG_DIS_VALUE;
}
...
void Cy_USBFS_Dev_Drv_Enable(USBFS_Type *base, cy_stc_usbfs_dev_drv_context_t const *context)
{
    ...
    USBFS_DEV_USBIO_CR1(base) |= USBFS_USBIO_CR1_USBPUEN_Msk;
...
static void Ep0IntrHandler(USBFS_Type *base, cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    if (0U != (ep0Cr & USBFS_EP0_CR_ACKED_TXN_Msk))
    {
        /* Checks the packet direction */
        if (_FLD2BOOL(USBFS_EP0_CR_SETUP_RCVD, ep0Cr))
        {
            /* A setup packet received */
            context->ep0CtrlState = CY_USBFS_DEV_DRV_EP0_CTRL_STATE_SETUP;

            /* Handles SETUP */
            if (_FLD2VAL(USBFS_EP0_CR_MODE, ep0Cr) == CY_USBFS_DEV_DRV_EP_CR_NAK_INOUT)
            {
                ...
                */
                ...
                if (false == _FLD2BOOL(USBFS_EP0_CR_SETUP_RCVD, ep0Cr))
                {
                ...
                else
                {
                    ...
                    Cy_USBFS_Dev_Drv_SetSieInterrupt(base, USBFS_USB_INTR_CAUSE_EP0_INTR_Msk);
                }
            }
        }
        /* Handles IN */
        else if (_FLD2BOOL(USBFS_EP0_CR_IN_RCVD, ep0Cr))
        {
            ...
            else if (CY_USBFS_DEV_DRV_EP0_CTRL_STATE_STATUS_OUT == context->ep0CtrlState)
            {
                ...
                Cy_USBFS_Dev_Drv_SetEp0Count (base, 0U, USBFS_EP0_CNT_DATA_TOGGLE_Msk);
        ...
        /* Handles OUT */
        else if (_FLD2BOOL(USBFS_EP0_CR_OUT_RCVD, ep0Cr))
        {
            if (CY_USBFS_DEV_DRV_EP0_CTRL_STATE_DATA == context->ep0CtrlState)
            {
                /* Data stage: invokes a callback to proceed the control transfer */
...
static void BusResetIntrHandler(USBFS_Type *base, cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    /* ID# 293217: write CR0 when pull-up is enabled */
    if((USBFS_DEV_USBIO_CR1(base) & (1u<<2)) != 0U)
    {
                                  ...
                                  cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    */
    ...
    inEndpoint   = (bool)CY_USBFS_DEV_DRV_IS_EP_DIR_IN(endpointData->address);
                                ...
                                cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    if (0U != (intrCause & USBFS_USB_INTR_CAUSE_ARB_EP_INTR_Msk))
    {
    ...
    /* SIE: Data endpoints */
    intrCauseEp = (intrCause >> USBFS_USB_INTR_CAUSE_EP1_INTR_Pos);
    ...
    /* SOF */
    if (0U != (intrCause & USBFS_USB_INTR_CAUSE_SOF_INTR_Msk))
    {
        SofIntrHandler(base, context);
    }

    /* Controls EP0 */
    if (0U != (intrCause & USBFS_USB_INTR_CAUSE_EP0_INTR_Msk))
    {
        Ep0IntrHandler(base, context);
    }

    /* Bus Reset */
    if (0U != (intrCause & USBFS_USB_INTR_CAUSE_BUS_RESET_INTR_Msk))
    {
                                    ...
                                    cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    if (NULL != buffer)
    {
        ...
        context->ep0DataToggle ^= (uint8_t) USBFS_EP0_CNT_DATA_TOGGLE_Msk;
    ...
    else
    {
        ...
        /* Updates the CNT and CR registers to continue the IN transfer */
        Cy_USBFS_Dev_Drv_SetEp0Count (base, numBytes, USBFS_EP0_CNT_DATA_TOGGLE_Msk);
...
void Cy_USBFS_Dev_Drv_Resume(USBFS_Type *base, cy_stc_usbfs_dev_drv_context_t *context)
{
    ...
    USBFS_DEV_USB_CLK_EN(base) = CY_USBFS_DEV_DRV_WRITE_ODD(USBFS_USB_CLK_EN_CSR_CLK_EN_Msk);
    ...
    /* ID# 337915: Restore response to LPM packets */
    lpmCtl = USBFS_DEV_LPM_CTL(base);
    USBFS_DEV_LPM_CTL(base) = lpmCtl;

...
/*----------------------------------------------------------------------------
  Internal References
 *----------------------------------------------------------------------------*/
#if (defined(__ICCARM__) && (defined(CY_DEVICE_PSC3) || defined(CY_DEVICE_PSC3M6)))
...
void FpuEnable_NS(void)
{
// Use __FPU_PRESENT, not __FPU_USED — secure side may use softfloat
// but NS domain still needs FPU enabled
#if defined (__FPU_PRESENT) && (__FPU_PRESENT == 1U)
    uint32_t interruptState = __get_PRIMASK();
    __disable_irq();
    SCB->NSACR |= (3UL << 10);                    /* grant NS access to CP10, CP11 */
    SCB_NS->CPACR |= SCB_CPACR_CP10_CP11_ENABLE;
    __DSB();
    __ISB();
    __set_PRIMASK(interruptState);
#endif /* (__FPU_PRESENT) && (__FPU_PRESENT == 1U) */
}
#endif
...
#endif
{
    ...
    #if defined (CY_DEVICE_PSC3) || defined (CY_DEVICE_PSC3_P8) || defined (CY_DEVICE_PSC3M6)
    /* Skip from NS context SRSS_PWR_CTL2 may be
     * inaccessible when PPC marks PROT_SRSS_SECURE as NS but HW overrides. */
    #if !defined(COMPONENT_NON_SECURE_DEVICE)
    SRSS_PWR_CTL2 &= ~_VAL2FLD(SRSS_PWR_CTL2_PORBOD_LPMODE, 1u); /* DRIVERS-21917 */
    #endif
    #endif /* defined (CY_DEVICE_PSC3) || defined (CY_DEVICE_PSC3_P8) */

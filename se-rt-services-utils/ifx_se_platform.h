...
*   \defgroup platform_image_hash Image Hash management
*       Image Hash management definitions
...
/* TODO: (SRTS-602) Remove these defines once IFX_SE_SYSCALL_HANDLER_ADDR moved to device headers */
#if defined(CY_IP_MXS22BTSS)
/* Syscall handler is placed at fixed address in S RT Services code */
#if defined(CY_IP_MXS22WIFI1X1AX20TBSS)
#if CY_IP_MXS22BTSS_VERSION == 1
    #define IFX_SE_SYSCALL_HANDLER_ADDR         0x120143a5u
#elif CY_IP_MXS22BTSS_VERSION == 3
    #define IFX_SE_SYSCALL_HANDLER_ADDR         0x120153a5u
#else
    #error "Unknown CY_IP_MXS22BTSS_VERSION value"
#endif /* CY_IP_MXS22WIFI1X1AX20TBSS */
#else
    #define IFX_SE_SYSCALL_HANDLER_ADDR         0x1200a3a5u
#endif /* CY_IP_MXS22WIFI1X1AX20TBSS */
#endif /* CY_IP_MXS22BTSS */
 ...
 *
 * It supports only asymmetric key algorithm based attestation (symmetric
 * attestation not supported).
 * It supports all required EAT claims that are defined in the PSA attestation
 * token specification for the new profile (https://www.ietf.org/archive/id/draft-tschofenig-rats-psa-token-20.txt).
 *
 ...
 * \param[out] token_length   The pointer to the actual length of the generated
 ...
 *
 * \param[in] challenge_size  The length of the challenge object (size_t).
 * \param[out] token_length   The pointer to the actual length of the
 *                            attestation token (size_t).
 * \param[in] ctx             The pointer to the SE syscall context that contain
 *                            a special syscall data (IPC release callback etc).
 *
 ...
 * for RRAM or SRAM memory region depending on platform.
 *
 * This function allows to configure only one MPC structure for each platform.
 * Either:
 * - RRAM Host port with permissions for PC>1. Can be called by PC=2 only and
 * does not allow to modify MPC configuration for the regions PC=2 does not
 * have access to.
 * - SRAM with permissions for PC>1. Does not allow to modify MPC configuration for the
 * S_RT_SERVICES region with the private data.
 *
...
/** \addtogroup platform_image_hash
 * \{
 */

/*******************************************************************************
 * Function Name: ifx_se_set_image_hash
 ***************************************************************************//**
 *
 * Set image hash by image ID, stores hash data into RRAM.
 *
 * \param[in] image_id     The image_id to set hash data
 *                         (0 to \ref IFX_SE_IMAGE_HASH_NUM - 1).
 * \param[in] hash         The pointer to the image hash data.
 * \param[in] hash_size    Size of image hash data (size_t).
 * \param[in] ctx          The pointer to the SE syscall context that contain
 *                         a special syscall data (IPC release callback etc).
 *
 * \return    \ref IFX_SE_SUCCESS for success or error code
...
ifx_se_status_t ifx_se_set_image_hash(ifx_se_fih_t image_id,
                                      const ifx_se_fih_ptr_t hash, ifx_se_fih_t hash_size,
                                      void *ctx);

/*******************************************************************************
 * Function Name: ifx_se_get_image_hash
 ***************************************************************************//**
 *
 * Reads image hash by image ID stored in RRAM.
 * Returns the actual hash size that was stored.
 *
 * After factory reset the zero-hash is returned with maximum hash size supported
 * by particular platform.  The \p hash_size parameter should be maximum supported hash
 * size for this case: either \ref IFX_SE_HASH_LENGTH(IFX_SE_ALG_SHA_256) or
 * \ref IFX_SE_HASH_LENGTH(IFX_SE_ALG_SHA_512).
 * This allows to detect that hash was never set.
 *
 * \param[in] image_id     The image_id to get hash data
 *                         (0 to \ref IFX_SE_IMAGE_HASH_NUM - 1).
 * \param[out] hash        The pointer to the image hash memory buffer.
 * \param[in]  hash_size   The size of the image hash memory buffer (size_t).
 * \param[out] hash_length The pointer to the actual size of the
 *                         image hash data in the memory buffer (size_t).
 * \param[in] ctx          The pointer to the SE syscall context that contain
 *                         a special syscall data (IPC release callback etc).
 *
 * \return    \ref IFX_SE_SUCCESS for success or error code
...
ifx_se_status_t ifx_se_get_image_hash(ifx_se_fih_t image_id,
                                      ifx_se_fih_ptr_t hash, ifx_se_fih_t hash_size,
                                      ifx_se_fih_ptr_t hash_length,
                                      void *ctx);

/*******************************************************************************
 * Function Name: ifx_se_verify_image_hash
 ***************************************************************************//**
 *
 * Verifies hash by image ID comparing provided image hash
 * with previously stored in RRAM.
 *
 * After factory reset the zero-hash is compared with the provided image hash with
 * maximum hash size supported by particular platform.
 * The \p hash_size parameter should be maximum supported hash
 * size for this case: either \ref IFX_SE_HASH_LENGTH(IFX_SE_ALG_SHA_256) or
 * \ref IFX_SE_HASH_LENGTH(IFX_SE_ALG_SHA_512).
 * This allows to detect that hash was never set.
 *
 * \param[in] image_id     The image_id to verify hash data
 *                         (0 to \ref IFX_SE_IMAGE_HASH_NUM - 1).
 * \param[in] hash         The pointer to the image hash data.
 * \param[in] hash_size    Size of image hash data (size_t).
 * \param[in] ctx          The pointer to the SE syscall context that contain
 ...
 *
 * \return    \ref IFX_SE_SUCCESS for success,
 *            \ref IFX_SE_SYSCALL_VERIFY_ERROR when hash verification failed
 *
 ******************************************************************************/
...
/*******************************************************************************
 * Function Name: ifx_se_lock_image_hash
 ***************************************************************************//**
 *
 * Locks the image hash by image ID stored in RRAM to prevent it from being changed
 * until device reset.
 *
 * After this function call SE RT Services can't modify the image hash
 * calling \ref ifx_se_set_image_hash function.
 *
 * \param[in] image_id     The image_id to lock hash data
 *                         (0 to \ref IFX_SE_IMAGE_HASH_NUM - 1).
 * \param[in] ctx          The pointer to the SE syscall context that contain
 *                         a special syscall data (IPC release callback etc).
 *

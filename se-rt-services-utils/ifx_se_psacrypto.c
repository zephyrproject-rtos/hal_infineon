...
ifx_se_status_t ifx_se_sign_hash(
                                    ...
                                    void *ctx)
 {
    ...
    if (ifx_se_is_key_id_pqc(key))
    {
        status = ifx_se_pqc_sign_hash(key, alg, hash, hash_length,
                                      signature, signature_size, signature_length, ctx);
                                    ...
                                    void *ctx)
{
    ...
    if (ifx_se_is_key_id_pqc(key))
    {
        status = ifx_se_pqc_verify_hash(key, alg, hash, hash_length, signature, signature_length);
                                 ...
                                 void *ctx)
{
    ...
    if (ifx_se_is_key_id_pqc(key))
    {
        return IFX_SE_SYSCALL_NOT_SUPPORTED;
    }
#endif
...
ifx_se_status_t ifx_se_destroy_key(ifx_se_key_id_fih_t key, void *ctx)
{
    ...
    if (ifx_se_is_key_id_pqc(key))
    {
        status = ifx_se_storage_destroy_key(key);
...
#if defined(COMPONENT_MW_WOLFSSL)
    if ((attributes != NULL) && ifx_se_is_key_type_pqc(attributes))
    {
        status = ifx_se_pqc_generate_key(attributes, key, ctx);
...
#if defined(COMPONENT_MW_WOLFSSL)
    if (ifx_se_is_key_type_pqc(attributes))
    {
        status = ifx_se_storage_import_key(attributes, data, data_length, key);
                                    ...
                                    void *ctx)
{
    ...
    if (ifx_se_is_key_id_pqc(key))
    {
        status = ifx_se_pqc_export_public_key(key, data, data_size, data_length);

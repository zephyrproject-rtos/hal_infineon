...
#define IFX_SE_SYSCALL_GROUP_IMAGE_HASH                    (0x3D6B6500u)
...
/* IFX_SE_SYSCALL_GROUP_IMAGE_HASH service IDs */
#define IFX_SE_SYSCALL_SET_IMAGE_HASH                      IFX_SE_SYSCALL_CODE(IFX_SE_SYSCALL_GROUP_IMAGE_HASH, 130)
#define IFX_SE_SYSCALL_GET_IMAGE_HASH                      IFX_SE_SYSCALL_CODE(IFX_SE_SYSCALL_GROUP_IMAGE_HASH, 131)
#define IFX_SE_SYSCALL_VERIFY_IMAGE_HASH                   IFX_SE_SYSCALL_CODE(IFX_SE_SYSCALL_GROUP_IMAGE_HASH, 132)
#define IFX_SE_SYSCALL_LOCK_IMAGE_HASH                     IFX_SE_SYSCALL_CODE(IFX_SE_SYSCALL_GROUP_IMAGE_HASH, 133)
...
/** Maximal number of images to store image hash data */
#define IFX_SE_IMAGE_HASH_NUM                  (8u)
...
/** Returned when hash verification failed */
#define IFX_SE_SYSCALL_VERIFY_ERROR            IFX_SE_ERROR_CODE(0xFF00001Du)
...
/** SE RT Services user defined syscall context is set to NULL in regular use case.
 *  It is passed as a value for an argument *ctx in ifx_se_ APIs since default implementation
 *  of functions ifx_se_syscall(), ifx_se_hal_syscall_lock(), ifx_se_hal_syscall_start(),
 *  ifx_se_hal_syscall_release() do not process it.
 *
 *  However, user may want to redefine mentioned functions, since they are declared as __WEAK,
 *  and utilize *ctx argument if each API to pass its own context structure to a syscall.
 *
 *  This is useful when library is used for example in a multitheaded environment. In this case
 *  *ctx may be used to pass data related to a particular thread, which triggers syscall.
 *  Redefined syscall processing functions may do additional job, taking to account info from *ctx.
 *
 *  Another use case may be execution profiling. For example, user can define functions to log time
 *  when entering syscall and then exiting syscall. Pointers to these functions are then put in
 *  structure, which is passed as a value to *ctx argument.
 *
 *  Basically any custom data may be defined as a structure and then passed to syscall *ctx argument.
 *  User then needs to decide where he wants to process this custom data and redefine one or several
 *  available syscall processing functions defined as __WEAK:
 *  - ifx_se_syscall()

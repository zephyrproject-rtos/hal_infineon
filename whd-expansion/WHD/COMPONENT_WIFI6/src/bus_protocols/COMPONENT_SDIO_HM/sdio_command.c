...
sdio_cmd_get_ip_info(sdio_command_t sdio_cmd, sdio_bcdc_header_t *cmd_bcdc)
{
    ...
    do {
        ...
        result = cy_wcm_get_ip_addr(CY_WCM_INTERFACE_TYPE_STA, &ip_addr);
        if (result == CY_RSLT_SUCCESS) {
            HM_CMD_MSG(("WCM GetIP address successful\n"));
        ...
        result = cy_wcm_get_ip_netmask(CY_WCM_INTERFACE_TYPE_STA, &ip_addr);
        if (result == CY_RSLT_SUCCESS) {
            HM_CMD_MSG(("WCM Netmask address successful\n"));
        ...
        result = cy_wcm_get_gateway_ip_address(CY_WCM_INTERFACE_TYPE_STA, &ip_addr);
        if (result == CY_RSLT_SUCCESS) {
            HM_CMD_MSG(("WCM Get gateway address successful\n"));
...
sdio_cmd_wl_cmd(sdio_command_t sdio_cmd, uint8_t *cmd_msg)
{
    ...
    HM_CMD_MSG(("WL cmd %ld, len %ld, set %lx\n",
...
sdio_cmd_scan(sdio_command_t sdio_cmd, sdio_bcdc_header_t *cmd_bcdc)
{
    ...
    if (result == CY_RSLT_SUCCESS) {
        HM_CMD_MSG(("Start Scan successful\n"));
    } else {
        HM_CMD_MSG(("Start Scan failed %lx\n", result));
    }
...
sdio_cmd_connect(sdio_command_t sdio_cmd, uint8_t *cmd_msg)
{
    ...
    HM_CMD_MSG(("Connect SSID: %s (len %d), security: 0x%lx, band: %d\n",
    ...
    */
    ...
    if (connect_param.ap_credentials.security != CY_WCM_SECURITY_OPEN) {
        ...
        HM_CMD_MSG(("auth type is set to CY_WCM_SECURITY_WPA2_AES_PSK\n"));
    }
    ...
    if (result == CY_RSLT_SUCCESS) {
        ...
        HM_CMD_MSG(("network Connection successful IP:%s \n", ip_addr_str));
    } else {
        HM_CMD_MSG(("network Connection failed %lx\n", result));
    }
}
...
sdio_cmd_disconnect(sdio_command_t sdio_cmd, sdio_bcdc_header_t *cmd_bcdc)
{
    ...
    if (result == CY_RSLT_SUCCESS) {
        HM_CMD_MSG(("network Disconnection successful\n"));
    } else {
        HM_CMD_MSG(("network Disconnection failed %lx\n", result));
    }
...
uint32_t sdio_cmd_at_read_data(uint8_t *buffer, uint32_t size)
{
    ...
    if (size < strlen(sdio_cmd->at_cmd_buf)) {
        HM_CMD_MSG(("buffer size %ld isn't enough to get AT command %d\n", size, strlen(sdio_cmd->at_cmd_buf)));
        return CYHAL_SDIO_RSLT_ERR_CONFIG;
    }

    HM_CMD_MSG(("AT CMD read data: %s\n", sdio_cmd->at_cmd_buf));
...
cy_rslt_t sdio_cmd_at_write_data(uint8_t *buffer, uint32_t length)
{
    ...
    HM_CMD_MSG(("AT CMD write data: %s\n", buffer));
...
static void
sdio_cmd_get_level(sdio_command_t sdio_cmd, sdio_bcdc_header_t *cmd_bcdc)
{
    sdio_cmd_uint32_t *rsp = NULL;
    uint8_t rsp_len = sizeof(*rsp);

    rsp = (sdio_cmd_uint32_t *)sdio_cmd_get_rsp(rsp_len, cmd_bcdc);
    if (!rsp) {
        return;
    }
...
static void
sdio_cmd_set_level(sdio_command_t sdio_cmd, uint8_t *cmd_msg)
{
    sdio_bcdc_header_t *cmd_bcdc = (sdio_bcdc_header_t *)cmd_msg;
    uint32_t *level = (uint32_t *)(cmd_msg + sizeof(*cmd_bcdc));
    sdio_bcdc_header_t *rsp = NULL;
    uint8_t rsp_len = sizeof(*rsp);
...
static void sdio_cmd_get(sdio_command_t sdio_cmd, uint8_t *cmd_msg)
{
    ...
    switch (bcdc->cmd)
    {
        ...
        case INF_C_IP_INFO:
            ...
            break;
        case INF_C_MSG_LEVEL:
            sdio_cmd_get_level(sdio_cmd, bcdc);
            break;
        default:
...
#endif /* defined(SDIO_HM_AT_CMD) */
        case INF_C_MSG_LEVEL:
            sdio_cmd_set_level(sdio_cmd, cmd_msg);
            break;
        default:

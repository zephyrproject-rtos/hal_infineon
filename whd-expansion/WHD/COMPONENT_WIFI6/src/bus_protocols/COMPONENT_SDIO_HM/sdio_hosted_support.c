...
static void sdio_hm_wcm_event_callback(cy_wcm_event_t event, cy_wcm_event_data_t *event_data)
{
    ...
    HM_EVENT_MSG(("Receive WCM event = %d\n", event));
...
void sdio_hm_int_evt_info(sdio_handler_t sdio_hm, sdiod_event_data_host_info_t       *host_info)
{
    ...
    if (host_info->io_enabled) {
        ...
        if (result != CY_RSLT_SUCCESS) {
            ...
            HM_INFO_MSG(("SDIO Deinit done, SDIO Lines put to tristate, put SDIO host into Shutdown\n"));
...
cy_rslt_t sdio_hm_deinit(sdio_handler_t sdio_hm)
{
    ...
    if (!sdio_hm->sdio_instance.hw_inited) {
        HM_INFO_MSG(("SDIO is in already deinitialized state \n"));
        return result;
    }
    ...
    HM_INFO_MSG(("sdio deinit complete\n"));
...
cy_rslt_t sdio_hm_reinit(void)
{
    ...
    HM_INFO_MSG(("SDIO reinit, putting back SDIO CLK, CMD, Data Lines on proper state\n"));
    ...
    if (sdio_hm->sdio_instance.hw_inited) {
        HM_INFO_MSG(("SDIO is in already Initialized state \n"));
        return result;
    }

    HM_INFO_MSG(("sdio re-init start\n"));
    ...
    HM_INFO_MSG(("sdio init complete\n"));
...
cy_rslt_t sdio_hm_init(void)
{
    ...
    HM_INFO_MSG(("SDIO for HM configured successfully\n"));
...
static void sdio_hm_rx_process(sdio_handler_t sdio_hm, uint8_t *payload, uint16_t length)
{
    ...
    HM_RX_MSG(("Rx seq: 0x%x, channel: %d\n", seq, channel));
...
{
    HM_INFO_MSG(("TIME %2d s, cnt ok %ld/ err %ld, %ld bytes, %ld Kbps\n",
        TEST_TOTAL_TIME / 1000, tp_test_ok, tp_test_err, tp_test_cnt,
        (tp_test_cnt >> 7) / (TEST_TOTAL_TIME / 1000)));
    HM_INFO_MSG(("Rx error, cnt hdr %ld/ data %ld\n",

...
#else
extern whd_result_t whd_bus_sdio_irq_enable(whd_driver_t whd_driver, whd_bool_t enable);
extern whd_result_t whd_bus_sdio_irq_register(whd_driver_t whd_driver);
extern whd_result_t whd_bus_sdio_enable_oob_intr(whd_driver_t whd_driver, whd_bool_t enable);
extern whd_result_t whd_bus_sdio_register_oob_intr(whd_driver_t whd_driver);
extern whd_result_t whd_bus_sdio_unregister_oob_intr(whd_driver_t whd_driver);
...
{
    /* Change this for OOB implementation with Zephyr */
    return false;
}
#endif
...
exit:
#ifdef WHD_CUSTOM_HAL
	whd_custom_hal_sdio_unmask_interrupt();
#endif /* WHD_CUSTOM_HAL */
#ifdef WHD_ZEPHYR
	whd_bus_sdio_irq_enable(whd_driver, WHD_TRUE);
#endif /* WHD_ZEPHYR */
...
whd_result_t whd_bus_sdio_read_frame(whd_driver_t whd_driver, whd_buffer_t *buffer)
{
    ...
    *buffer = NULL;
    ...
    if (result != WHD_SUCCESS)
    {
#if ((defined(WHD_CUSTOM_HAL) && defined(IMXRT)) || defined(WHD_ZEPHYR))
        (void)whd_bus_sdio_abort_read(whd_driver, WHD_TRUE);    /* ignore return - not much can be done if this fails */
#else
...
#ifdef WHD_CUSTOM_HAL
        result = whd_hal_sdio_host_bulk_transfer(whd_driver->bus_priv->sdio_obj, (whd_hal_sdio_host_transfer_type_t)direction, arg.value, mode,
                                     (uint32_t *)aligned_local_buffer, data_size, response);
#else
        result = whd_hal_sdio_host_bulk_transfer(whd_driver->bus_priv->sdio_obj, (whd_hal_sdio_host_transfer_type_t)direction, arg.value,
                                     (uint32_t *)aligned_local_buffer, data_size, response);
#endif /* WHD_CUSTOM_HAL */

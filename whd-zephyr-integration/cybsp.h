...
/* WiFi node */
#define WHD_SDIO_NODE               DT_COMPAT_GET_ANY_STATUS_OKAY(infineon_cyw43xxx_wifi_sdio)

/* WiFi control pins */
#if !defined(CYBSP_WIFI_WL_REG_ON)
	#if DT_NODE_HAS_PROP(WHD_SDIO_NODE, wifi_reg_on_gpios)
		#define CYBSP_WIFI_WL_REG_ON \
	DT_GET_CYHAL_GPIO_FROM_DT_GPIOS(WHD_SDIO_NODE, wifi_reg_on_gpios)
	#else
	...
	#if DT_NODE_HAS_PROP(WHD_SDIO_NODE, wifi_host_wake_gpios)
		#define CYBSP_WIFI_HOST_WAKE \
	DT_GET_CYHAL_GPIO_FROM_DT_GPIOS(WHD_SDIO_NODE, wifi_host_wake_gpios)
	#else
	...
	#if DT_NODE_HAS_PROP(WHD_SDIO_NODE, wifi_dev_wake_gpios)
		#define CYBSP_WIFI_DEV_WAKE \
	DT_GET_CYHAL_GPIO_FROM_DT_GPIOS(WHD_SDIO_NODE, wifi_dev_wake_gpios)
	#else
